"""
Static asset availability tests for structa.cloud using django-grep test utilities.

Mirrors ctc-research.com/core/CI/tests/test_assets_selenium.py,
adapted for the `alliance-website` container and structa.cloud static paths.
...
def _staticfiles_root() -> Path:
    """Return the staticfiles output directory for structa.cloud."""
...
class StructaStaticAssetAvailabilityTest(AssertHTMLMixin, BaseTestCase):
    """
    Verify static assets are served correctly for structa.cloud.

    Validates: Requirement 2 — static asset verification (structa mirror).
    """
    ...
    def test_static_root_directory_contains_css_files(self):
        ...
        self.assertGreater(len(css_files), 0, f"No .css files found under {static_root}")
    ...
    def test_static_root_directory_contains_js_files(self):
        ...
        self.assertGreater(len(js_files), 0, f"No .js files found under {static_root}")
