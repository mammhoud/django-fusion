...
def test_preservation_alliance_template_unchanged():
    """
    Property 2: Preservation - Alliance Template Unchanged
    ...
    **Validates: Requirement 3.1**
    ...
    assert 'wagtail_i18n_tags' in features['loaded_libraries'], (
        f"Alliance template must have 'wagtail_i18n_tags' loaded. "
    ...
    assert features['uses_get_translation'], (
        f"Alliance template must use 'get_translation' tag. "
        f"This is the working reference implementation that must remain unchanged."
    )

    # Assert: All core features must be present
    assert features['has_language_form'], "Alliance template must have language switching form"
    assert features['has_flag_icons'], "Alliance template must have flag icons"
    assert features['has_current_language_indicator'], "Alliance template must have current language indicator"
    assert features['has_translated_url'], "Alliance template must have translated URL navigation"
...
if __name__ == '__main__':
    ...
    print("Running: test_preservation_alliance_template_unchanged")
    print("-" * 70)
    try:
        test_preservation_alliance_template_unchanged()
        print("✓ PASS: Alliance template works correctly")
    except AssertionError as e:
        print(f"✗ FAIL: {e}")
    print()
