...
class CertificateService:
    ...
    def validate_certificate(
        certificate_id: str, issuer: str = None, recipient_name: str = None
    ) -> Tuple[bool, str, Any]:
        """
    ...
    def get_certificate_profile(user) -> Dict[str, Any]:
        ...
        if dashboard_data is None:
            ...
            by_issuer = list(
                certificates.values("issuer").annotate(count=Count("id")).order_by("-count")[:5]
            )

            # Get expiring certificates
            expiring_list = list(
                expiring_soon.values("name", "issuer", "expiry_date").order_by("expiry_date")[:10]
            )
            ...
            dashboard_data = {
                ...
                "recent_certificates": list(certificates.order_by("-issue_date")[:5].values()),
            }
    ...
    def generate_certificate_report(
        content_object, start_date: datetime = None, end_date: datetime = None
    ) -> Dict[str, Any]:
        """
        ...
        certificates = Certificate.objects.filter(
            content_object=content_object, issue_date__range=[start_date, end_date]
        )
        ...
        issuer_stats = list(
            ...
            .annotate(count=Count("id"), avg_duration=Avg(F("expiry_date") - F("issue_date")))
            .order_by("-count")[:10]
        )
        ...
        return {
            ...
            "summary": {
                ...
                "valid_percentage": round(
                    (valid_certificates / total_certificates * 100)
