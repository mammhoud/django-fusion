...
```bash
cp compose/config/secrets.example.yml compose/config/secrets.yml
```
...
secrets:
  postgres_password:
    file: ./compose/config/secrets.yml
  django_secret_key:
    file: ./compose/config/secrets.yml
```
...
```bash
SECRETS_FILE=./compose/config/secrets.yml
```
...
secrets_file = os.getenv('SECRETS_FILE', './compose/config/secrets.yml')
...
```bash
# Backup (encrypted)
gpg --symmetric compose/config/secrets.production.yml

# Restore
gpg compose/config/secrets.production.yml.gpg > compose/config/secrets.production.yml
```
...
- Ensure file has correct permissions: `chmod 600 compose/config/secrets.yml`
