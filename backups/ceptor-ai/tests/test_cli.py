from ceptor_ai.cli import main, package_info


def test_package_info_uses_expected_import_name() -> None:
    assert package_info()["import_package"] == "ceptor_ai"
...
def test_projects_command(capsys) -> None:
    ...
    assert '"package": "ceptor-ai"' in out


def test_rseal_plan_command(tmp_path, capsys) -> None:
    sample = tmp_path / "sample.py"
    sample.write_text("from ceptor_ai.ai.integrations import AIIntegration\n", encoding="utf-8")
    assert main(["rseal-plan", str(tmp_path)]) == 0
    assert '"status": "move-to-ceptor-ai"' in capsys.readouterr().out
