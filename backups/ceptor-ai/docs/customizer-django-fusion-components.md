...
5. Site URL configuration includes the site routes under a prefix such as `/fusion/` or `/customizer/`.
...
Place site-specific registration in a site module such as `applications/ctc-research/www/projects/site.py` or equivalent. The exact import path can vary by site.
