...
**1.1** The spec MUST document, in prose hint-note form, how the monorepo separates per-site concerns from shared concerns. Specifically: the three site entry points (`ctc-research/assets/static/js/app.js`, `lms-demo/assets/static/js/app.js`, `VResume/assets/static/js/app.js`) are per-site; the shared core lives in `assets/static/js/projects/main.js` (used by ctc-research and lms-demo; VResume carries its own core).
