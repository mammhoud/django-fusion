#!/usr/bin/env bash
# Phase 3: Move crafts_ai.seeder → django_fusion.seeder
...
echo "=== Moving seeder: crafts_ai → django_fusion ==="
...
sed -i 's/from crafts_ai\.seeder/from django_fusion.seeder/g' "$DST"/*.py
sed -i 's/import crafts_ai\.seeder/import django_fusion.seeder/g' "$DST"/*.py
...
import warnings
warnings.warn(
    "crafts_ai.seeder is deprecated. Use django_fusion.seeder instead.",
    DeprecationWarning,
    stacklevel=2,
)
