...
Both sites use `AuthHTMXAdapter` to integrate allauth with the HTMX fragment system. See [Auth Adapter](../reference/auth/adapter.md).
...
allauth templates are overridden via `AuthHTMXAdapter.TEMPLATE_MAP`. See [Auth Templates](../reference/auth/templates.md) for the full mapping.
