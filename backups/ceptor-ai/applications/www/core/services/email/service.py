...
Consolidated from the per-site duplicates at
``applications/<site>/www/projects/handlers/services/email/service.py``
(identical across ctc-research + lms-demo) and the parallel
``applications/<site>/www/apps/services/email/service.py`` companions.

The site-specific copies have been removed; this single class now
powers ``applications/tasks/email.py`` for all three websites.
"""
...
class EmailService:
    ...
    def queue(
        ...
        Preserved verbatim from the per-site originals (byte-identical
        ctc-research + lms-demo behavior): imports the local plugin
        ``send_email_task`` so existing Celery task rows / queues keep
        resolving to the same target function after consolidation.
        """
        from ceptor_ai.services.infrastructure.jobs import dispatch_job
        from applications.www.worker.email_tasks import (
            send_email_task,
        )
