"""Unit test locking in `ctc-research` tests' `_django_settings.configure()` idempotency contract.
...
1. **DJANGO_SETTINGS_MODULE unset** — `configure()` (default) should
   apply `TEST_SETTINGS` (`SECRET_KEY == "ctc-research-test-secret-key"`).
...
Why `--project <workspace_root>` (the workspace `applications/`)?
  - `ctc-research/pyproject.toml` declares no runtime dependencies;
...
if not (_TEST_WORKSPACE_ROOT / "pyproject.toml").exists():
    raise RuntimeError(
        ...
        "expected ctc-research parent to be the workspace root."
    )
...
# ── Sentinel fake parent module name (ctc-research-specific) ─────────────────
_FAKE_PARENT_SETTINGS = "_fake_parent_settings_for_ctc_research_idempotency_test"


# ── Sentinel SECRET_KEY per site (matches `tests/_django_settings.TEST_SETTINGS`) ──
_TEST_SETTINGS_SECRET_KEY = "ctc-research-test-secret-key"
