"""Single source of truth for ctc-research test Django settings.
...
`django-fusion/tests/_django_settings.py`, `lms-demo/tests/_django_settings.py`,
...
Extraction rationale (mirrors `applications/libs/django-fusion/tests/_django_settings.py`):
  ...
  - Centralize ctc-research test settings in one discoverable module so
...
#
# `ctc-research` is the source-of-truth test settings file for ITS test
# suite. `lms-demo` and `VResume` carry their own copies mirroring this
# one — the duplication is intentional (each site's test infra is
...
TEST_SETTINGS: dict = {
    ...
    "MIDDLEWARE": [],
    "ROOT_URLCONF": "",
    "SECRET_KEY": "ctc-research-test-secret-key",
    "STATIC_URL": "/static/",
    "USE_TZ": True,
