...
make docker-run     # Start container (http://tinker.localhost:5073)
...
#### `make docker-build`
**Purpose:** Build Docker image for tinker.
...
...
=> exporting to image
=> naming to docker.io/library/tinker:latest
```

**Example:**
```bash
make docker-build
# → Creates tinker:latest image
```

---

#### `make docker-run`
**Purpose:** Start tinker in Docker container.

**What it does:**
1. Starts container from tinker:latest
2. Runs database migrations
3. Exposes port 5073 via Traefik
4. Mounts volumes for data persistence

**Access:**
- Via Traefik: http://tinker.localhost:5073
...
**What it does:**
- Stops running tinker container
- Preserves data in volumes (tinker-data, tinker-static, tinker-media)

**Example:**
```bash
make docker-stop
# Stopping tinker ... done
```
...
```bash
make docker-logs
# tinker  | 🚀 Running Django system checks...
# tinker  | ✅ System check identified no issues (0 silenced).
# tinker  | 🚀 Applying migrations...
```
...
```bash
make docker-shell
# root@container:/app/tinker# python manage.py shell
# >>>
```
...
5. Outputs to assets/bundles/tinker/

**Output files:**
```
assets/bundles/tinker/
...
# ✅ Bundles cleaned: assets/bundles/tinker/
```
...
```bash
make collectstatic
# 145 static files copied to '/app/tinker/staticfiles'.
```
...
```bash
make check-deploy
# (1_0.W001) All ALLOWED_HOSTS should include 'tinker.example.com'
```
