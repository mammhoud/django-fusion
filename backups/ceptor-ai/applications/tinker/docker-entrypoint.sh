...
# The Docker compose mounts a named volume at /app/tinker/data/, but
# Django's default DB path is /app/tinker/db.sqlite3. Create a symlink
...
case "${1:-server}" in
    server)
        ...
        echo "🚀 Starting gunicorn on 0.0.0.0:${TINKER_PORT}..."
        exec gunicorn \
            --bind "0.0.0.0:${TINKER_PORT}" \
            --workers "${WORKERS}" \
