"""Single source of truth for VResume test Django settings.
...
and `ctc-research/tests/_django_settings.py` — the contract test
...
Extraction rationale (mirrors `applications/libs/django-fusion/tests/_django_settings.py`):
  ...
  - Centralize VResume test settings in one discoverable module so
...
#
# `VResume` is the source-of-truth test settings file for ITS test
# suite. `lms-demo` and `ctc-research` carry their own copies mirroring
...
TEST_SETTINGS: dict = {
    ...
    "MIDDLEWARE": [],
    "ROOT_URLCONF": "",
    "SECRET_KEY": "vresume-test-secret-key",
    "STATIC_URL": "/static/",
    "USE_TZ": True,
