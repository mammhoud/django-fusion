...
# Fusion site for routable components (django-fusion Site → Application hierarchy)
...
urlpatterns = [
    # Health checks (from django-fusion)
...
# ── Routable Components (django-fusion) ────────────────────────────────────
# Must be mounted BEFORE Wagtail's catch-all in i18n_patterns so routes
# are checked first. Otherwise Wagtail eats app routes with 404.
urlpatterns += [path("", include((vresume_site.urls[0], vresume_site.urls[1]), namespace=vresume_site.urls[2]))]
