"""Unit test locking in `lms-demo` tests' `_django_settings.configure()` idempotency contract.
...
1. **DJANGO_SETTINGS_MODULE unset** — `configure()` (default) should
   apply `TEST_SETTINGS` (`SECRET_KEY == "lms-demo-test-secret-key"`).
...
Why `--project <workspace_root>` (the workspace `applications/`)?
  - `lms-demo/pyproject.toml` declares `dependencies = []` (empty), so
    `uv run --project <site>` would not pull Django into the sandbox.
    The workspace `applications/pyproject.toml` is the source of truth
...
(double-underscore prefix + suffix, internal sentinel). See the helper
...
if not (_TEST_WORKSPACE_ROOT / "pyproject.toml").exists():
    raise RuntimeError(
        ...
        "expected lms-demo parent to be the workspace root."
    )
...
# ── Sentinel fake parent module name (lms-demo-specific) ──────────────────────
...
# side-channel files.
_FAKE_PARENT_SETTINGS = "_fake_parent_settings_for_lms_demo_idempotency_test"


# ── Sentinel SECRET_KEY per site (matches `tests/_django_settings.TEST_SETTINGS`) ──
_TEST_SETTINGS_SECRET_KEY = "lms-demo-test-secret-key"
...
def _run_configure_subprocess(
    *,
    ...
    script = textwrap.dedent("""
        ...
        #    `sys.modules` BEFORE django tries to autoconfigure.
    ...
    # Sentinel control vars (server also checks for these).
...
def test_configure_applies_test_settings_when_django_settings_module_unset():
    ...
    matches `_django_settings.TEST_SETTINGS.SECRET_KEY`.
    """
