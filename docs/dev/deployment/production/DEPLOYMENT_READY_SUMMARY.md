...
| **lms** | 19 chunks | ✅ Compiled | `lms/assets/static/js/app.js` | ✅ Merged & Unified |
...
- `/root/site/websites/lms/assets/bundles/lms/bundles.json` (114 lines)
...
Services Ready:
  ...
  ✅ lms-website       - Django app (port 5071)
...
- `lms/assets/static/js/usecase-config.js`
...
# LMS Demo
webpack --entry lms/assets/static/js/app.js
...
```bash
docker compose -f docker-compose.yml up -d \
  ctc-research-website lms-website vresume-website
```

### Step 3: Verify Deployments
```bash
# Check container status
docker ps | grep -E 'ctc-research|lms|vresume|traefik|postgres|redis'
...
📦 Bundles:
  ...
  - lms/assets/bundles/lms/bundles.json
...
2. **Restore certificates** (if needed):
   ...
   bash /root/site/websites/compose/traefik/cert-backup.sh restore \
     /root/site/websites/compose/traefik/certs/[backup-timestamp]_certs
   ```
