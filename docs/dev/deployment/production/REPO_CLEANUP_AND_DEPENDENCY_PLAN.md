...
2. **Removed tracked log output**
   - Removed committed runtime logs from `logs/`, `compose/logs/`, `lms/logs/`, and generated `.log` files under `docs/`.
...
7. **Documented website apps**
   - Added `docs/WEBSITE_APPLICATION_INVENTORY.md` with the apps discovered for `ctc-research`, `lms`, and `VResume`.
...
2. Run `manage.py check` for all websites: `ctc-research`, `lms`, and `vresume`.
...
- `lms`: startup fails because `crafts_ai.models.settings.subscription.NewsletterSubscription` requires `settings.PROFILE_MODEL`, but `lms` settings do not define it. Add the same explicit `PROFILE_MODEL` used by `ctc-research` or wire the real profile model if the site has one.
