...
- `libs/django-fusion/` — local reusable django-fusion package and routable component primitives.
...
`django-fusion` supplies routable viewset and site primitives from `libs/django-fusion/`. A site project registers one or more application-level viewsets, and each application exposes page viewsets or fragment viewsets.
...
The examples below are illustrative and should be adapted to the exact classes available in the site being changed. Keep shared framework code in `libs/django-fusion/` and site-specific code in the appropriate canonical site path.
