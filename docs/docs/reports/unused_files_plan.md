...
2. Do not install `www.apps`, `plugins.accounts`, `plugins.profile`, `plugins.products`, `plugins.lms`, or `plugins.blog` until package imports are updated away from removed `django_osoul.core.*` and `django_rseal.http.*` paths.
...
- Replace `django_osoul.core.models` with current `django_osoul.models` exports where available.
- Replace `django_osoul.core.managers` with current `django_osoul.managers` exports where available.
- Replace `django_osoul.core.services` with current `django_osoul.services` exports where available.
- Replace `django_rseal.http.*` imports with the current `django_rseal.workflows.*`, `django_rseal.services.*`, or package-documented paths.
- Keep `django_grep` limited to tests and health/debug URLs.
