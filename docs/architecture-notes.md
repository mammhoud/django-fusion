...
- **Three site directories**: `ctc-research/`, `lms-demo/`, `VResume/`
...
- ApexCharts, ECharts: Used by lms-demo and ctc-research
...
- `ctc`, `ctc-research.com`, `ctc-website` → `ctc-research`
- `structa`, `lms`, `lms-demo` → `lms-demo`
- `vresume`, `VResume`, `resume` → `VResume`

For ctc-research and lms-demo, three entry points are emitted:
- `main`: Shared core
- `static`: CSS + vendor
- `app`: Site-specific
...
Both ctc-research and lms-demo resolve their Django settings through:
```python
