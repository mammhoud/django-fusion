...
This case study analyzes both patterns, evaluates django-fusion's current integration points, and proposes a unified architecture where django-fusion components serve both rendered HTML (via `fusion_render_first`) and bolt-style JSON APIs.
...
Make every django-fusion `RoutableComponent` optionally expose a django-bolt API endpoint with zero boilerplate, while preserving the existing `bolt_view` adapter for backward compatibility.

### 5.2 New Module: `django_fusion.bolt`
...
```python
# In any fusion project (lms-fusion, cms-fusion, etc.)
...
1. Create `django_fusion.bolt` module with `FusionBoltAPI`
2. Add `@fusion_endpoint` decorator
3. Auto-generate OpenAPI schemas from fusion components
4. Add `FusionBoltAuthBackend` for session→bolt auth bridge
