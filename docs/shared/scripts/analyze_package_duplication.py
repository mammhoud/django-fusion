...
Scans django-osoul and django-rseal packages to detect code duplication (≥70% similarity threshold).
...
class PackageDuplicationAnalyzer:
    """
    Scans django-osoul and django-rseal packages for code similarity and duplication.
    Uses AST-based comparison for semantic similarity.
    """
    ...
    def _categorize_duplication(self, file_a: str, file_b: str) -> Tuple[str, str, str]:
        """Categorize duplication between django_osoul and django_rseal packages."""
        # Determine which package each file belongs to
        is_a_in_osoul = 'django-osoul' in file_a
        is_b_in_osoul = 'django-osoul' in file_b
        is_a_in_rseal = 'django-rseal' in file_a
        is_b_in_rseal = 'django-rseal' in file_b
        ...
        if is_a_in_osoul and is_b_in_osoul:
            ...
            return "belongs-in-rseal", "django_rseal", self._determine_module_path(file_a)
    ...
    def _determine_module_path(self, file_path: str) -> str:
        ...
        elif 'django-rseal' in file_path:
            rel_path = file_path.split('django-rseal/')[1]
        else:
    ...
    def generate_report(self) -> str:
        ...
        report_lines = [
            ...
            "",
            "## Phase 6.10: Package Deduplication — Remove Duplication Between django_osoul and django_rseal",
            "",
        ...
        for i, result in enumerate(sorted_results, 1):
            ...
            if result.category == "belongs-in-osoul":
                ...
                report_lines.append(f"- Move to `django_rseal.{result.target_module}`")
                report_lines.append(f"- Remove duplicate from other location")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Analyze code duplication between django_osoul and django_rseal packages')
    ...
    package_b = 'venv/libs/django-rseal/'
