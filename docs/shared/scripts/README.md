...
python3 scripts/check_boundaries.py --rule ceptor-ai-no-django
...
#### `check_boundaries.py`
- `--rule`: Specific boundary rule to check (ceptor-ai-no-django, osoul-no-wagtail, rseal-no-projects, grep-test-only)
