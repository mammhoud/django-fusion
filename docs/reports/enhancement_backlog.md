...
| High | Add CI smoke checks for Docker build and compose configuration. | Docker deployment plans required stable deployment verification. | CI runs `docker compose ... config`, builds `applications/compose/Dockerfile`, and fails on missing workspace paths. |
...
- Actual workspace member names are enforced in `pyproject.toml`, `manage.py`, and `applications/compose/Dockerfile`.
