...
| Thin application layer | Site apps subclass or configure reusable behavior from `django-osoul`, `django-rseal`, and `django-grep`. | Business rules stay reusable and easier to patch across sites. |
...
flowchart TD
    ...
    Selector --> CTC[ctc-research site directory]
...
- **Shared libraries are external contracts**: `django-osoul`, `django-rseal`, and `django-grep` must remain importable in the active environment before URL import checks and runtime checks can pass.
