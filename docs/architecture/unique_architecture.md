...
| Thin application layer | Site apps subclass or configure reusable behavior from `django-osoul`, `crafts-ai`, and `django-osoul`. | Business rules stay reusable and easier to patch across sites. |
...
- **Shared libraries are external contracts**: `django-osoul`, `crafts-ai`, and `django-osoul` must remain importable in the active environment before URL import checks and runtime checks can pass.
