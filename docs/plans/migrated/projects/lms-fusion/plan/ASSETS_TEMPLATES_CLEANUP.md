...
**Master plan:** [`docs/plans/README.md`](../../../../README.md)
**Date:** 2026-07-26
...
- [ ] `make test WEBSITE=lms-fusion` (targeted backend suite passes; dispatcher target still pending)
...
- [x] Smoke test on the running frontend (frontend build and route smoke checks pass; live service check requires Docker stack)
...
>   label change.
> - Run the project dispatcher target `make test WEBSITE=lms-fusion`.
> - Address the 17 Django system-check warnings (treebeard compatibility and
>   `DEFAULT_AUTO_FIELD` in `django_fusion` models).
> - Start the full Docker stack and verify live media HTTP responses; compose
>   and image-build checks do not replace that deployment smoke test.
>
...
- [ ] Test the full stack locally with `docker compose up` (compose configuration and image builds validated; live startup requires external networks/services)

#### Phase 8 — Shared Media / Static Serving
- [x] Configure uploaded media to route to the global `shared-media` Nginx container.
- [x] Configure the shared media volume mount in the `lms-fusion` backend container and
  `shared-media` (live HTTP verification remains pending).
- [ ] Keep frontend source assets build-time only; do not expose
  `lms-fusion/assets/` directly.
- [x] Document which URLs route to Next.js, Django, or `shared-media`.
  Static/media mounts and shared-media location blocks are configuration-validated;
  live HTTP routing remains part of the pending full-stack smoke test.
