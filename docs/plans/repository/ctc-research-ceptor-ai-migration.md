...
> **Tags:** #precis-ctc #ceptor-ai #migration

**Date:** July 30, 2026  
**Scope:** ~80+ files across `projects/precis/precis-ctc/` (formerly `projects/precis/precis-ctc/`)  
**Goal:** Remove all ceptor-ai imports, migrating to native models, Wagtail blocks, and django-fusion equivalents.

> **Status: ✅ COMPLETE (code level).** Verified 2026-08-16: zero `ceptor_ai` Python
> imports remain anywhere under `projects/precis/` (main, lnd-structa, lms-ctc,
> configs). `ceptor_ai` is absent from `INSTALLED_APPS`, `settings.py`, and
> `pyproject.toml`; `libs/ceptor-ai` no longer exists in the checkout. The
> newsletter confirm/unsubscribe routes (Section 4) are registered. See
> **Section 6 — Remaining leftovers** for non-import references still present.
...
The most pervasive import. precis-ctc uses `ceptor_ai.models.Person` as its profile model.
...
precis-ctc uses ceptor-ai's custom Wagtail blocks extensively for page content.
...
## 4. Newsletter URL Routing Fix — ✅ DONE

The missing routes are now registered in `backend/apps/pages/urls.py`:

| URL | Status |
|-----|--------|
| `auth/newsletter/subscribe/` | ✅ `NewsletterSubscribeView` (POST) |
| `newsletter/confirm/<str:token>/` | ✅ `NewsletterConfirmView` (GET) |
| `newsletter/unsubscribe/<str:token>/` | ✅ `NewsletterUnsubscribeView` (GET + POST) |

These match the URLs generated by `apps/domain/services/communication/newsletter.py`
...
3. **The `www/` vs `plugins/` duplication** — precis-ctc has parallel code in both `www/apps/` and `plugins/` directories; the migration should consolidate into one canonical location
4. **django-fusion does NOT provide blocks** — No `django_fusion.contrib.blocks` exists; all blocks must be local to precis-ctc
...
## 6. Remaining leftovers (non-import — no runtime impact)

The code-level migration is complete. These are the only remaining `ceptor`
references under `projects/precis/`, none of which are imports:

| Location | Kind | Notes |
|----------|------|-------|
| `main/backend/Makefile` + `lms-ctc/backend/Makefile` | ✅ Cleaned 2026-08-16 | Removed dead `libs/ceptor-ai/src` from `PYTHONPATH` and `--upgrade-package ceptor-ai` from `install-internal`. |
| `backend/apps/domain/**/README.md` (48 files across `main` + `lms-ctc`) | Stale doc stubs | Auto-generated provenance docs that still say "Part of ceptor-ai" and show `from ceptor_ai.* import …` examples. Harmless but misleading; safe to delete or rewrite as native docs. |
| `frontend/` + `backend/apps/pages/seed_pages.py` + `backend/apps/core/management/commands/setup_wagtail_home.py` | Product marketing | Legitimate — ceptor-ai remains a shipped product (Syntara consumes it); the landing site lists it as a hidden catalog item. Keep. |

**Completion checks:**

- [x] `ceptor_ai` Python imports → **0** across `projects/precis/`
- [x] `ceptor_ai` in `INSTALLED_APPS` / `settings.py` / `pyproject.toml` → **none**
- [x] Newsletter confirm/unsubscribe routes registered (Section 4)
- [x] `libs/ceptor-ai` directory removed
- [x] Dead `ceptor-ai` references removed from `main` + `lms-ctc` backend Makefiles
- [ ] Optional: delete/rewrite the 48 `apps/domain/**/README.md` provenance stubs
