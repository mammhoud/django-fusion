...
> **Status (14 Aug 2026): COMPLETE locally.** The shop API contract suite
> (`backend/tests/test_orders_api.py` + `test_shop_api.py`) and the My Orders
> fragment (`my_orders_fragment`) are implemented and verified. The canonical
> directory is **`projects/formints/formint-client/`** (this plan's `formint-client/`
> paths are the legacy alias for the same tree).
>
> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Complete the pos-client edition (`formint-client/`) by documenting its design, architecture, and data model (the separate branch of the extension chain) and hardening it with shop-API test coverage and a green verification gate.
...
- All changes live under `projects/formints/formint-client/` only.
...
- Test commands: Django `cd projects/formints/formint-client/backend && python -m pytest` · client `cd projects/formints/formint-client && pnpm lint && pnpm build`.
...
**Files:**
- Create: `formint-client/backend/tests/__init__.py`
- Create: `formint-client/backend/tests/test_shop_api.py`
- Test runner: `cd projects/formints/formint-client/backend && python -m pytest tests/ -q`
...
Create `formint-client/backend/tests/__init__.py` (empty) and `formint-client/backend/tests/test_shop_api.py`:
...
> The `reverse()` names above assume the `name=` values in `shop/urls.py`. If the url names differ, read `formint-client/backend/shop/urls.py` and use the actual names — keep the assertions (200 + JSON contains "Latte") unchanged.

- [x] **Step 2: Run test to verify it fails**

Run: `cd projects/formints/formint-client/backend && python -m pytest tests/test_shop_api.py -q`
...
- [x] **Step 3: Adjust to the real contract (if the failure was only names)**
...
Run: `cd projects/formints/formint-client/backend && python -m pytest tests/ -q`
Expected: PASS.

- [x] **Step 5: Commit**

```bash
git add projects/formints/formint-client/backend/tests/
...
- [x] **Step 1: Run the Vue client lint + build**

Run: `cd projects/formints/formint-client && pnpm lint && pnpm build`
Expected: oxlint passes; `vue-tsc && vite build` produces `dist/`.

- [x] **Step 2: Run the storefront check**

Run: `cd projects/formints/formint-client/frontend && pnpm build` (or the `astro check` equivalent in `frontend/package.json`)
Expected: build succeeds.

- [x] **Step 3: Fix any failure found (only if a check fails)**
...
```bash
git add projects/formints/formint-client/
...
- [x] **Step 1: Confirm the pos-client section is accurate**
...
**Files:**
- Create: `formint-client/playwright.config.ts`
- Create: `formint-client/e2e/storefront.spec.ts`
...
- [x] **Step 1: Write the config + spec**

Create `formint-client/playwright.config.ts` (mirror `formintA/playwright.config.ts`, baseURL `http://127.0.0.1:8000` — the shop backend's dev port, or the port `backend/manage.py runserver` uses):
...
**Files:**
- Modify: `formint-client/backend/shop/views.py` (add `my_orders_fragment` mirroring `products_fragment`)
- Modify: `formint-client/backend/shop/urls.py` (wire `/fragments/my-orders/`)
- Test: `formint-client/backend/tests/test_shop_api.py` (add fragment test)
...
Append to `formint-client/backend/tests/test_shop_api.py`:
...
In `formint-client/backend/shop/views.py`, add (mirroring the structure of `products_fragment`, using the same template directory and naming convention):
...
In `formint-client/backend/shop/urls.py`, next to the other fragment routes, add:

```python
path("fragments/my-orders/", my_orders_fragment, name="shop_my_orders_fragment"),
```

Create the template `formint-client/backend/shop/templates/shop/fragments/my_orders.html` listing `{{ order.id }}`, `{{ order.total }}`, and status, following the markup style of the existing fragment templates.
...
```bash
git add formint-client/backend/shop/views.py formint-client/backend/shop/urls.py formint-client/backend/shop/templates/shop/fragments/my_orders.html formint-client/backend/tests/test_shop_api.py
