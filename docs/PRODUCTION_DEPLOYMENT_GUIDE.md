...
docker images | grep -E "(ctc-research|lms-demo|vresume|traefik|postgres|redis|shared-media)"
```
...
docker compose -f docker-compose.yml up -d web-ctc-research web-lms-demo web-vresume
...
docker exec web-lms-demo python manage.py migrate --noinput 2>&1 | tail -5
...
docker exec web-lms-demo python manage.py collectstatic --noinput 2>&1 | tail -3
...
docker exec web-lms-demo python manage.py loaddata fixtures/initial_data.json 2>/dev/null || echo "No initial_data.json found"

# 3. Create admin users
echo "[7.3] Creating admin users..."
echo "  - CTC Research: Run: docker exec web-ctc-research python manage.py createsuperuser"
echo "  - LMS Demo: Run: docker exec web-lms-demo python manage.py createsuperuser"
...
docker compose -f docker-compose.yml logs --tail 50 web-lms-demo | grep -i error || echo "No errors in LMS logs"
...
docker exec web-lms-demo python manage.py migrate --noinput >> $LOG_FILE 2>&1
...
docker compose exec web-lms-demo python manage.py migrate --noinput
...
docker compose exec web-lms-demo python manage.py collectstatic --noinput
...
docker exec web-lms-demo python manage.py dbshell -c "SELECT 1;"
