...
- `/root/site/websites/lms-demo/docker-compose.yml` (if applicable)
...
# Build lms-demo/structa (Tailwind + SCSS + JS)
...
docker exec web-lms-demo python manage.py collectstatic --noinput --site lms-demo 2>&1 | tee logs/collectstatic-lms.log
...
docker exec web-lms-demo python manage.py populate_content --locale en
...
docker exec web-lms-demo python manage.py check --deploy
...
make verify-runtime-site WEBSITE=lms-demo
