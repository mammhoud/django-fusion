...
The local libraries in `core/libs/` are Git submodules and must be initialized before the sites can boot.
...
- **Site directory not found**: ensure `WEBSITE` matches a canonical site name in `core/cli.py`.
