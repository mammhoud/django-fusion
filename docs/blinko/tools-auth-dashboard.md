---
title: Tools Dashboard (tools-web)
description: Vanilla Node.js tools dashboard with Alpine + htmx, sign-in modal, per-tool unlock gate, and theme toggle
navigation:
...
> **Created:** 2026-08-25 · **Rewritten:** 2026-08-26 (vanilla migration)
...
This vanilla Node.js service (no framework — replaces the former Astro SSR app) provides:

1. **Tools dashboard** at `/` — lists every self-hosted tool under `application/tools/`
2. **Sign-in modal** — authentication happens in a modal on the dashboard (no separate `/login` page; `/login` redirects to `/`)
3. **Per-tool unlock gate** — clicking a tool asks for a shared unlock password (`TOOLS_UNLOCK_PASSWORD` from the environment); on success the tool opens in the same tab
4. **Theme toggle** — light/dark switch persisted in `localStorage`, defaults to the system preference
5. **Session management** — signed HTTP-only cookies (HMAC)
6. **API endpoints** — `/api/login`, `/api/logout`, `/api/session`, `/api/unlock`
