...
`projects/lms-fusion/configs/base/auth.py` (shared across all sites):
...
| `projects/lms-fusion/configs/base/apps.py` | Added `"allauth.mfa"` to `THIRD_PARTY_APPS` |
| `projects/cms-fusion/configs/base/auth.py` | `MFA_PASSKEY_LOGIN_ENABLED` → `True`; added `"webauthn"` to `MFA_SUPPORTED_TYPES` |
| `projects/lms-fusion/configs/base/auth.py` | Same as above |
