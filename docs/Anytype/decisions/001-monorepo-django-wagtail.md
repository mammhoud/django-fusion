...
The project is organized as a single repository with a shared `projects/` directory containing all sites. A canonical `projects/Makefile` dispatches per-site commands via `WEBSITE=<name>` variables. Shared settings live in `projects/configs/`, shared templates in `projects/assets/templates/`, and shared static files in `projects/assets/static/`.
...
**Risks:**
- **Monorepo sprawl** — mitigated by consistent directory conventions and Makefile discipline
