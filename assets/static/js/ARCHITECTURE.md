...
This document describes the JavaScript module architecture for the application. The architecture is designed for modularity, dynamic loading, and centralized configuration.
...
│   ├── navigations/     # Navigation tracking and tracking
...
3. **Registry & Component Discovery**
   - `registry.init()`: Initializes core services like Events, Cookies, and SSE.
   - `uiManager.init()`: Scans the DOM for components (defined in `UI_COMPONENTS`) and initializes them.

## Core Managers

### Application (`app.js`)
The main orchestrator. It manages the high-level lifecycle and coordinates between managers.
- **Singleton:** `app`
...
Configuration is centralized in `init.config.js` and uses the `CONFIG` object. It includes:
...
- **page:** (Runtime) Holds the detected page type and discovered components.
...
For debugging, key instances are exposed to the `window` object in development mode:
- `window.app`: The application instance.
- `window.registry`: The infrastructure registry.
- `window.uiManager`: The UI component manager.
- `window.APP_CONFIG`: The active configuration.
