...
**Scope:** `proxy/traefik/*` and `proxy/docker-compose.traefik.yml` (the local
...
**Staging strategy:** 3-stage rollout with `proxy/traefik/dynamic/certs.yml`
...
The self-signed certs in `proxy/certs.yml` remain loaded, so if anything
...
git checkout -- proxy/traefik/dynamic/vresume.yml
...
In `proxy/traefik/dynamic.yml`, **remove** the staging line:
...
Restart: `docker compose -f proxy/docker-compose.traefik.yml restart default-proxy`.
...
- `proxy/traefik/dynamic/structa-cloud.yml`
  - `structa-site-https`
  - `structa-media-https`
- `proxy/traefik/dynamic/media-servers.yml`
  - `shared-media-https`
  - `ctc-media-https`
  - `lms-media-https`
  - `vresume-media-https`
- `proxy/traefik/dynamic/dashboard.yml`
...
After Stage 3, the only cert path is `proxy/acme/acme.json`.

---

## Local compose

Already wired in `proxy/docker-compose.traefik.yml`:

- `./acme:/etc/traefik/acme:rw` volume mount
- `CF_DNS_API_TOKEN`, `CF_API_EMAIL`, `CF_API_KEY`, `LETSENCRYPT_EMAIL`
  env vars sourced from `proxy/.env` (or `${VAR:-}` defaults)
...
1. In the Coolify UI for `default-proxy`:
   ...
   - **Sync** the `proxy/traefik/dynamic.yml` and `proxy/traefik/dynamic/*.yml`
