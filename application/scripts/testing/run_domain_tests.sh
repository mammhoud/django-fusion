...
test_configuration() {
  ...
  if grep -q "structa.cloud@gmail.com" /home/structa.cloud/application/proxy/.env 2>/dev/null; then
...
test_traefik_config() {
  ...
  router_count=$(find /home/structa.cloud/application/proxy/configs/traefik/dynamic -name '*.yml' -o -name '*.yaml' 2>/dev/null | wc -l)
...
test_acme_setup() {
  ...
  if [ -f "/home/structa.cloud/application/proxy/configs/acme.json" ]; then
    log_success "ACME storage file exists"
    perms=$(stat -c %a /home/structa.cloud/application/proxy/configs/acme.json)
    if [ "$perms" = "600" ]; then
  ...
  if [ -f "/home/structa.cloud/application/proxy/configs/traefik/dynamic.yml" ]; then
    if grep -q "certificatesResolvers:" /home/structa.cloud/application/proxy/configs/traefik/dynamic.yml; then
