...
HELPER_IMAGE="${REGISTRY_URL:-ghcr.io}/coollabsio/default-helper:${LATEST_HELPER_VERSION}"
...
nohup bash -c "
    ...
    for container in coolify postgres redis default-realtime; do
    ...
    docker run -v /data/coolify/source:/data/coolify/source -v /var/run/docker.sock:/var/run/docker.sock \${DOCKER_CONFIG_MOUNT} --rm \${REGISTRY_URL:-ghcr.io}/coollabsio/default-helper:\${LATEST_HELPER_VERSION} bash -c \"LATEST_IMAGE=\${LATEST_IMAGE} docker compose --env-file /data/coolify/source/.env \${COMPOSE_FILES} up -d --remove-orphans --wait --wait-timeout 60\" >>\"\$LOGFILE\" 2>&1
