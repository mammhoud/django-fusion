...
| AFFiNE server and migration | `application/proxy/docker-compose.nginx.yml` | `application/proxy/affine-data/` + shared `affine` database |
| PostgreSQL and Redis | `application/databases/` | database volumes |
| Public TLS and routing | `application/proxy/` | Traefik ACME storage |
