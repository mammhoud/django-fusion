...
#
# Commands:
#   bootstrap-acme         Create application/proxy/configs/acme.json (0600) placeholder
...
#
# See application/proxy/LETSENCRYPT.md for the full HTTP-01 deployment runbook.
################################################################################
...
# Keep these in sync with the per-site ALLOWED_HOSTS in projects/<site>/docker-compose.yml.
DOMAINS=(
...
################################################################################
# Bootstrap ACME storage (application/proxy/configs/acme.json, mode 0600)
################################################################################
...
bootstrap_acme() {
  ...
  log_info "You can now start the proxy: docker compose -f application/proxy/docker-compose.traefik.yml up -d"
}
...
Primary (Let's Encrypt / ACME) commands:
  bootstrap-acme         Create application/proxy/configs/acme.json (mode 0600) before first start
...
Examples:
  # First-time LE setup
  cp application/proxy/.env.example application/proxy/.env  # optional: set LETSENCRYPT_EMAIL
...
Let's Encrypt Configuration (HTTP-01, no DNS tokens):
  See application/proxy/LETSENCRYPT.md for the full runbook (DNS prerequisites,
