...
The template customizer application has been successfully renamed from "customizer" to "tinker" and fully documented for deployment and operation. The application is a Django-based AI chat and template discovery tool that integrates with Structa Cloud's multi-site architecture.
...
|------|-----------|-----------|
| `settings.py` | `WEBSITE_NAME = "templatetinker"` | `WEBSITE_NAME = "tinker"` |
| `settings.py` | `WEBSITE_IDENTIFIER = "templatetinker"` | `WEBSITE_IDENTIFIER = "tinker"` |
| `docker-compose.yml` | `container_name: customizer` | `container_name: tinker` |
| `docker-compose.yml` | `image: customizer:latest` | `image: tinker:latest` |
| `docker-compose.yml` | `CUSTOMIZER_*` env vars | `TINKER_*` env vars |
| `Dockerfile` | All paths: `customizer/` | All paths: `tinker/` |
| `docker-entrypoint.sh` | `APP_HOME="/app/customizer"` | `APP_HOME="/app/tinker"` |
...
TINKER_ALLOWED_HOSTS=tinker.example.com
```
...
### Docker
- **Volume:** `tinker-data:/app/tinker/data/`
...
3. View Docker logs: `docker-compose logs tinker`
...
1. **What does tinker do?** → `README.md`
