...
Canonical import: from django_fusion.management.managers.role_hierarchy import RoleHierarchyManager
...
class RoleHierarchyManager:
    ...
    # Parent roles inherit permissions from child roles. Sites may override
    # these maps for a different authorization policy.
    ...
    ROLE_PERMISSIONS: dict[str, list[str]] = {
        ...
        "user": ["auth.view_user", "auth.view_group"],
    }
    ...
    def get_role_hierarchy(self, role: str) -> List[str]:
        """Return an ordered, duplicate-free hierarchy starting at *role*."""
        hierarchy: list[str] = []
        visited: set[str] = set()
        ...
        def visit(current: str) -> None:
            ...
            for child_role in self.ROLE_HIERARCHY.get(current, []):
                visit(child_role)
