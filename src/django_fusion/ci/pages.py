...
class SectionSummary:
    """A reusable template section or component discovered for a page."""
...
class PageCatalog:
    ...
    The catalog prefers Wagtail page records when the Wagtail app and database
    are available. It also scans configured template roots so customizer tools
    still work in projects that only render Django templates.
    """
    ...
    def wagtail_pages(self) -> list[PageSummary]:
        ...
        try:
            ...
            return []
        try:
            records = Page.objects.live().public().specific().defer_streamfields()
    ...
    def sections_for_template(self, template: str | None) -> tuple[SectionSummary, ...]:
        paths = self.component_paths_for_template(template)
        return tuple(
            SectionSummary(
    ...
    def component_paths_for_template(self, template: str | None) -> tuple[str, ...]:
        if not template:
            return ()
        paths = [p for p in self._included_templates(template) if self._is_component_path(p)]
    ...
    def get_template_roots(self) -> list[TemplateRoot]:
        ...
        for idx, root in enumerate(roots):
            if isinstance(root, TemplateRoot):
                ...
                result.append(TemplateRoot(name=f"templates-{idx}", path=Path(root)))
        return result
    ...
    def _roots_from_django(self) -> list[TemplateRoot]:
        ...
        for backend in engines.all():
            ...
            for directory in getattr(engine, "dirs", []):
                roots.append(TemplateRoot(name=Path(directory).name, path=Path(directory)))
        return roots
    ...
    def _included_templates(self, template: str) -> tuple[str, ...]:
        for root in self.get_template_roots():
            ...
            if candidate.exists():
                ...
                return tuple(INCLUDE_RE.findall(text))
        return ()
    ...
    def _customizer_url(
        self, *, path: str, template: str | None, root: TemplateRoot | None = None
    ) -> str:
