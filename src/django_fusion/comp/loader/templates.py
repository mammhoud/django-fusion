...
def get_template_directories() -> Generator[Path, Any, None]:
    from django_fusion.comp.plugins import pm
...
def get_component_directories(
    ...
    return [
        ...
        for component_dir in _settings.get_component_directory_names()
    ]


def gather_block_tag_template_usage() -> Generator[tuple[Path, set[str]], Any, None]:
    template_dirs = get_template_directories()
...
class NodeVisitor:  # pragma: no cover
    ...
    def visit_Template(self, template: Template, context: Context) -> None:  # noqa: N802
    ...
    def visit_BlockNode(self, node: BlockNode, context: Context) -> None:  # noqa: N802
        component_name = node.name.strip("\"'")
        self.components.add(component_name)
        self.generic_visit(node, context)

    def visit_ExtendsNode(self, node: ExtendsNode, context: Context) -> None:  # noqa: N802
        parent_template = node.get_parent(context)
        self.visit(parent_template, context)
        self.generic_visit(node, context)

    def visit_IncludeNode(self, node: IncludeNode, context: Context) -> None:  # noqa: N802
        try:
