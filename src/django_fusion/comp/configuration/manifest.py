    ...
    if hasattr(settings, "STATIC_ROOT") and settings.STATIC_ROOT:
        ...
        if manifest_path.exists():
            try:
                ...
                logger.warning(
                    f"Error reading asset manifest at {manifest_path}: {e!s}. Falling back to registry."
                )
                return None
...
def generate_asset_manifest() -> dict[str, list[str]]:
    ...
    """
    from django_fusion.comp.templates.templates import gather_block_tag_template_usage
