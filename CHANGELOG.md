...
## 2026-08-02 — precis-lms container recovery: mounts, migrations, templates

### docker (precis-lms + cms-fusion compose)

- Fixed stale container mounts that shadowed the shared configs package with an
  empty `projects/precis-lms/configs/` dir (caused `ModuleNotFoundError: No module
...
**precis-ctc** (~90 files, 44 imports, Phases 1-4 complete):
...
- 0 ceptor_ai imports remaining; project moved to `archives/precis-ctc/`

**cms-fusion & precis-lms**: Already clean (0 imports); newsletter native from start
...
### test: cms-fusion + precis-lms — 113 tests each, 0 failures
