...
- **Docker Compose:** `http://tinker.localhost:5073` (via Traefik)
- **Production:** `https://tinker.structa.cloud` (via Traefik with HTTPS)
...
**URL Parameters:**
- `website_slug` (required) – One of: `ctc-research`, `lms-demo`, `VResume`
...
**Location:** Loads from `.kilo/agent/*.json` in project root.
...
See `tinker/docs/postman-collection.json` for a Postman collection of all API endpoints.
