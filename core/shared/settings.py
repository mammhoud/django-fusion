"""Settings entrypoint for the sentinel `shared` site.

Mirrors the per-site settings.py pattern (see `core/ctc-research/settings.py`)
but with two differences:

1. No `_SITE_APP_DIR = _SITE_DIR / "www"`. The shared stack runs the
   `www.worker` package globally — registered once via
   `core/configs/base/apps.py` — so no per-site `www/` override is
   needed. Adding `_SITE_APP_DIR` here would *block* `www.core` and
   `www.worker` imports because the shared stack doesn't ship a
   per-site `www/` directory of its own.

2. Calls `configure_site_environment("shared", module="CMS",
   default_port=5080)` instead of a tenant site. This seeds the
   `DJANGO_WEBSITE`, `WEBSITE`, `SITE_DOMAIN`, `MODULE`, `PORT`, etc.
   env vars that `core/configs/settings/conf.py:MainSettings` reads
   via `default_factory=active_website_name` at import time.

Once this module is importable from runtime `sys.path`, it loads as
`settings` when `DJANGO_SETTINGS_MODULE=settings` is the default
(resolves to /app/shared/settings.py under the `PROJECT_PATH=shared`
bake) and as `shared.settings` when both `PROJECT_PATH=shared` (which
COPY-bakes `/app/shared/__init__.py`) and
`DJANGO_SETTINGS_MODULE=shared.settings` are set. With either path
resolvable, celery-beat and shared-worker's `python manage.py rundramatiq`
calls resolve cleanly without the historical `Unknown site 'shared'`
rejection that came from `core/cli.py:resolve_site` and
`core/configs/site.py:_normalise_website`.

DEV-TIME ONLY:
Production workers run with PROJECT_PATH=ctc-research baked at build time
in `core/compose/Dockerfile` (override-able via the
`TASKS_PROJECT_PATH=shared` build arg passed from
`applications/compose/docker-compose.tasks.yml:23`). Under the default
PROJECT_PATH=ctc-research, `python manage.py rundramatiq` is invoked with
DJANGO_SETTINGS_MODULE=settings resolving to
`core/ctc-research/settings.py` (bind-mounted at runtime) — NOT this module.

Setting TASKS_PROJECT_PATH=shared IS the runtime wiring: the
Dockerfile's `COPY core/${PROJECT_PATH}` step bakes shared/ contents
into /app/shared/, which the runtime loads as the default `settings`
module (DJANGO_SETTINGS_MODULE=settings resolves to
/app/shared/settings.py when /app/shared/ is on sys.path). Under the
default bake, this file is loaded by:
  - ad-hoc dev invocations: `python core/shared/__main__.py check`
  - unit tests under `tests/unit/` that patch `shared.settings` (or
    `DJANGO_SETTINGS_MODULE=shared.settings`)
  - any future tooling that explicitly sets DJANGO_SETTINGS_MODULE=shared.settings
"""

import sys
from pathlib import Path

# ── Site dir layout ──────────────────────────────────────────────────────
_SITE_DIR = Path(__file__).resolve().parent          # = core/shared/
_WORKSPACE_DIR = _SITE_DIR.parent                    # = core/

# Same REVERSED iteration pattern as per-site settings.py: put the
# per-site directory (here, `core/shared/`) at sys.path[0] so a
# subsequent `import shared.xxx` resolves to THIS directory rather
# than any sibling `shared/` package on the workspace path.
for _path in (str(_WORKSPACE_DIR), str(_SITE_DIR)):
    if _path in sys.path:
        sys.path.remove(_path)
    sys.path.insert(0, _path)

# ── Site tracker ─────────────────────────────────────────────────────────
from configs.site import configure_site_environment

configure_site_environment("shared", module="CMS", default_port=5080)

# ── Shared Django settings ──────────────────────────────────────────────
from configs.settings import *  # noqa: E402,F401,F403

# Explicit override of the per-app site identifier. `configs.settings`
# already arrived with these defaults filled in via the configs layer,
# but we re-assert them to make the intent visible in this file —
# future readers shouldn't have to chase down the registry to verify
# which site "shared" identifies as.
WEBSITE_NAME = "shared"

# No LOCAL_APPS appended here on purpose. The `www.worker` Celery /
# Dramatiq app is registered globally in
# `core/configs/base/apps.py:INSTALLED_APPS`. Site-specific apps that
# `core/<site>/settings.py` append (wagtail pages, plugins, etc.) are
# intentionally NOT added — the shared stack is site-agnostic and
# task dispatches it performs cross sites via the explicit queue
# routing inside `www.worker.tasks`.
