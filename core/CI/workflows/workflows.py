...
class UserOnboardingWorkflow:
    ...
    async def run(self, input: OnboardingWorkflowInput) -> dict:
        ...
        email_result = await workflow.execute_activity(
            ...
            EmailPayload(
                to=input.email,
                subject=f"Welcome to prj, {input.name}!",
                body=f"Hello {input.name},\n\nWelcome to prj! We're excited to have you on board.",
            ),
            start_to_close_timeout=timedelta(seconds=30),
            retry_policy=RetryPolicy(
                initial_interval=timedelta(seconds=1),
        ...
        followup_result = await workflow.execute_activity(
            ...
            EmailPayload(
                to=input.email,
                subject="How are you liking prj?",
                body=f"Hi {input.name},\n\nJust checking in to see how you're finding prj. Let us know if you have any questions!",
            ),
            start_to_close_timeout=timedelta(seconds=30),
            retry_policy=RetryPolicy(
                initial_interval=timedelta(seconds=1),
