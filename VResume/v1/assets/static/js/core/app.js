import { URLTrackerMixin } from '../lib/url.js';
import { moduleManager } from '../lib/module-manager.js';
import { NotificationSystem } from '../services/notifications.js';
import { UIManager } from '../components/manager.js';
...
export class Application extends URLTrackerMixin(class { }) {
    ...
    async init() {
        if (this.initialized) {
            if (this.config.debug) {
                console.debug('Application already initialized, skipping re-initialization');
            }
            return this;
        }
        ...
        try {
            ...
            if (this.config.notifications) {
                const notificationSystem = new NotificationSystem(this.config.notifications);
    ...
    setupErrorHandling() {
        ...
        window.addEventListener('error', (event) => {
            ...
            if (event.error && typeof event.error.message === 'string') {
                ...
                // Ignore bootbox message validation errors (we handle these in main.js)
                if (msg.includes('message') && msg.includes('null or an empty string')) {
                    console.debug('ℹ️ Bootbox validation error (handled)', msg);
                    event.preventDefault();
                    return;
                }
                
                // Ignore stack overflow errors (focus recursion - handled globally)
                if (msg.includes('Maximum call stack size exceeded')) {
                    console.debug('ℹ️ Stack overflow error (focus recursion - prevented)', msg);
                    event.preventDefault();
                    return;
                }
            }
        ...
        window.addEventListener('unhandledrejection', (event) => {
            ...
            if (typeof event.reason === 'string') {
                if (event.reason.includes('message') && event.reason.includes('null or an empty string')) {
                    console.debug('ℹ️ Bootbox validation error (handled)', event.reason);
                    event.preventDefault();
                    return;
                }
            }
    ...
    showNotification(message, type = 'info', options = {}) {
        ...
        const notificationSystem = this.managers.notifications;
