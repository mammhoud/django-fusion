...
SITE_NAME = "Structa"
DEFAULT_FROM_EMAIL_FALLBACK = "support@structa.cloud"
...
def _get_sender_accounts():
    ...
    while True:
        ...
        if email and password:
            accounts.append({"email": email, "password": password, "name": SITE_NAME})
        n += 1
...
def _resolve_template(template_type: str, fallback_template: str, context: dict):
    ...
    subject = context.get("subject", SITE_NAME)
...
def send_registration_email(user, confirmation_url: str) -> bool:
    ...
    context = {
        ...
        "expiration_hours": 24,
        "support_email": getattr(settings, "DEFAULT_FROM_EMAIL", DEFAULT_FROM_EMAIL_FALLBACK),
    }
    ...
    if not sender_accounts:
        ...
        for i, account in enumerate(sender_accounts):
            try:
                ...
                logger.error(f"Failed to send sign-in success email to {user.email} via sender {i + 1}: {e}")
                continue
        if not result:
...
def _send_with_django_backend(
    ...
    try:
        from_email = getattr(settings, "DEFAULT_FROM_EMAIL", DEFAULT_FROM_EMAIL_FALLBACK)
