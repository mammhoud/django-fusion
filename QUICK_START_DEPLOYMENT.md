...
docker exec web-lms-demo python manage.py collectstatic --noinput
...
# LMS Demo
docker exec -it web-lms-demo python manage.py createsuperuser --no-input \
  --username=admin --email=admin@lms-demo.com
docker exec web-lms-demo python manage.py shell <<EOF
from django.contrib.auth.models import User
u = User.objects.get(username='admin')
u.set_password('admin123')
u.save()
...
ls -lh /root/site/websites/lms-demo/assets/bundles/ | head -10
...
docker exec web-lms-demo python manage.py shell -c "from django.db import connection; print('✓ DB connected')"
