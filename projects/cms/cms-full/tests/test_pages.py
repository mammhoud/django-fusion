"""Tests for the static page content API on CTC Research."""
...
class TestPageDetail:
    ...
    def test_page_detail_returns_home(self):
        request = RequestFactory().get("/api/pages/home/")
        response = page_detail(request, "home")

        assert response.status_code == 200
        data = json.loads(response.content)
        assert data["slug"] == "home"
        assert data["title"] == "CTC Research — Advancing Clinical Trials"

    def test_page_detail_returns_about(self):
        request = RequestFactory().get("/api/pages/about/")
        response = page_detail(request, "about")

        assert response.status_code == 200
        data = json.loads(response.content)
        assert data["slug"] == "about"
        assert data["title"] == "About CTC Research"

    def test_page_detail_returns_services(self):
        request = RequestFactory().get("/api/pages/services/")
        response = page_detail(request, "services")

        assert response.status_code == 200
        data = json.loads(response.content)
        assert data["slug"] == "services"
        assert data["title"] == "Our Services"

    def test_page_detail_returns_team(self):
        request = RequestFactory().get("/api/pages/team/")
        response = page_detail(request, "team")
    ...
    def test_page_detail_not_found(self):
        request = RequestFactory().get("/api/pages/unknown/")
...
class TestPageFragment:
    ...
    def test_page_fragment_all_slugs(self):
        """Every CTC Research STATIC_PAGES slug returns a valid fragment pointer."""
        for slug in STATIC_PAGES:
            ...
            payload = json.loads(response.content)
            assert payload["data"]["page_slug"] == slug
            assert payload["data"]["title"] == STATIC_PAGES[slug]["title"]
...
class TestPageData:
    ...
    def test_false_query_param_returns_json(self):
        ...
        request = RequestFactory().get("/api/pages/home/data/?fusion_render_first=false")
        response = page_data(request, "home")
    ...
    def test_true_query_param_returns_html(self):
        ...
        request = RequestFactory().get("/api/pages/home/data/?fusion_render_first=true")
        response = page_data(request, "home")
    ...
    def test_can_decode_encoded_field(self):
        ...
        assert decoded["slug"] == "about"
        assert decoded["title"] == STATIC_PAGES["about"]["title"]
