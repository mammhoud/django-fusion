/**
 * Integration tests for the lms-fusion EventsPage.
 *
 * Covers:
...
describe('EventsPage (lms-fusion)', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    vi.useFakeTimers();
  ...
  it('shows free badge for free events in carousel', () => {
    ...
    // React Workshop and Community Meetup are free
  ...
  it('shows location or Online indicator', () => {
    renderWithProviders(<EventsPage />);
    // Locations appear in both carousel and grid
  ...
  it('shows prices for paid events', () => {
    ...
    const prices299 = screen.getAllByText('$299.99');
    expect(prices299.length).toBeGreaterThan(0);
    const prices199 = screen.getAllByText('$199.99');
  ...
  it('shows empty state when no events', () => {
    ...
    renderWithProviders(<EventsPage />);
    // Carousel should not render when slides are empty
