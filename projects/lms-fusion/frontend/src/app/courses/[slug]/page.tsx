...
export default function CourseDetailPage() {
  ...
  React.useEffect(() => {
    (async () => {
      try {
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5076/api'}/courses/${slug}`);
  ...
  if (loading) return (
    <div className="min-h-screen bg-white">
      <div className="max-w-6xl mx-auto px-4 py-16 space-y-6">
        ...
        <div className="bg-gray-200 h-12 w-1/2 rounded animate-pulse" />
  ...
  if (error || !course) return (
    <div className="min-h-screen flex items-center justify-center bg-white">
      <div className="text-center">
        ...
        <Link href="/courses" className="text-teal-600 hover:text-teal-800 font-medium">← Back to Courses</Link>
      </div>
    </div>
  );
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-col lg:flex-row gap-8 items-start">
            ...
            <div className="lg:w-80 bg-white text-gray-900 rounded-xl p-6 shadow-lg">
              ...
              <button className="w-full py-3 bg-teal-600 text-white font-medium rounded-lg hover:bg-teal-700 transition-colors mb-3">
                Enroll Now
              </button>
                        ...
                        {mod.lessons.map(lesson => (
                          <div key={lesson.id} className="flex items-center justify-between py-2 px-3 bg-white rounded-lg text-sm">
                            <div className="flex items-center gap-2">
                              ...
                              {lesson.is_preview && <span className="text-xs text-teal-600 font-medium">Preview</span>}
                            </div>
