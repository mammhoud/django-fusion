...
export default function BlogPostPage() {
  ...
  React.useEffect(() => {
    (async () => {
      try {
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5076/api'}/blog/${slug}`);
  ...
  if (loading) return (
    <div className="min-h-screen bg-white">
      <div className="max-w-4xl mx-auto px-4 py-16 space-y-6">
        ...
        <div className="bg-gray-200 h-12 w-3/4 rounded animate-pulse" />
    ...
    <div className="min-h-screen bg-white">
      ...
      <article className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <nav className="flex items-center gap-2 text-sm text-gray-500 mb-6">
          ...
          {post.categories?.[0] && (
            <>
              <Link href={`/blog?category=${post.categories[0].slug}`} className="hover:text-teal-700">{post.categories[0].name}</Link>
              <span>/</span>
            </>
          )}
        ...
        <div className="flex flex-wrap items-center gap-3 mb-4">
          {post.categories.map((cat) => (
            <Link key={cat.slug} href={`/blog?category=${cat.slug}`}
              className="px-3 py-1 bg-teal-100 text-teal-700 rounded-full text-xs font-medium hover:bg-teal-200">
              {cat.name}
            </Link>
          ))}
        ...
        <div className="prose prose-lg max-w-none prose-headings:text-gray-900 prose-a:text-teal-600 prose-img:rounded-xl" dangerouslySetInnerHTML={{ __html: post.content }} />
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {post.related_posts.map((rp) => (
                <Link key={rp.slug} href={`/blog/${rp.slug}`} className="group block bg-gray-50 rounded-xl overflow-hidden hover:bg-gray-100 transition-colors">
                  ...
                  <div className="p-4">
                    <h3 className="font-semibold text-gray-900 group-hover:text-teal-700 transition-colors line-clamp-2">{rp.title}</h3>
