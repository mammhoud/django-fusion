...
export default function BlogPage() {
  ...
  const fetchPosts = async (page = 1) => {
    setLoading(true);
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5076/api'}/blog?page=${page}&per_page=9`);
      ...
      <section className="bg-gradient-to-r from-teal-600 to-cyan-700 text-white py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <p className="text-lg text-teal-100 max-w-2xl mx-auto">
            Insights, tutorials, and updates from the Fusion LMS team
          </p>
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        {loading ? (
            ...
            {Array.from({ length: 6 }).map((_, i) => (
              <div key={i} className="bg-white rounded-xl shadow-sm overflow-hidden">
                ...
                <div className="p-6 space-y-3">
                  <div className="bg-gray-200 h-4 w-1/4 rounded animate-pulse" />
          ...
          <div className="text-center py-16">
            ...
            <button onClick={() => fetchPosts()} className="px-4 py-2 bg-teal-600 text-white rounded-lg hover:bg-teal-700">Retry</button>
          </div>
        ) : posts.length === 0 ? (
                ...
                <Link key={post.id} href={`/blog/${post.slug}`} className="group block bg-white rounded-xl shadow-sm overflow-hidden hover:shadow-md transition-shadow">
                  ...
                  <div className="p-6">
                    <div className="flex items-center gap-2 text-xs text-gray-500 mb-2">
                      {post.categories?.[0] && (
                        <span className="px-2 py-0.5 bg-teal-100 text-teal-700 rounded-full">{post.categories[0].name}</span>
                      )}
                      <span>{post.reading_time} min read</span>
                    </div>
                    <h2 className="text-lg font-semibold text-gray-900 group-hover:text-teal-700 transition-colors line-clamp-2 mb-2">
                      {post.title}
                    </h2>
