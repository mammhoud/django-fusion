...
/**
 * Content Rendering E2E Tests — LMS-Fusion Frontend
 *
 ...
 *
 * These tests require the LMS backend to be running.
 */
...
test.describe('API-Backed Content Rendering', () => {
  ...
  for (const { slug, title } of navPages) {
    test(`${title} (/${slug}) renders with API content`, async ({ page }) => {
      ...
      // Wait for content to settle (FusionProxy may need time to render)
      await page.waitForTimeout(1000);

      // Check HTTP status
      expect(response?.status(), `${title} should return 200`).toBe(200);

      // Body should have meaningful content (not just the skeleton/loading state)
      const bodyText = await page.textContent('body');
...
test.describe('Dynamic Page Content Verification', () => {
  test('navigating between pages shows correct content', async ({ page }) => {
    ...
    // Click the second nav link (skip home)
  ...
  test('homepage hero section renders content', async ({ page }) => {
    test.skip(pages.length === 0, 'No pages from API — skipping');

    await page.goto('/', { waitUntil: 'domcontentloaded', timeout: 10000 });
    await page.waitForTimeout(1000);

    // Check for hero section with content
    const hero = page.locator('.section-hero, .hero, [class*="hero"]');
    const heroCount = await hero.count();

    if (heroCount > 0) {
      const heroText = await hero.first().textContent();
      expect(heroText?.length ?? 0, 'Hero section should have text content').toBeGreaterThan(10);
    } else {
      // If no specific hero class, just ensure main content area exists
      const main = page.locator('main, #main, [role="main"]');
      const mainCount = await main.count();
      if (mainCount > 0) {
        const mainText = await main.first().textContent();
        expect(mainText?.length ?? 0, 'Main content area should have text').toBeGreaterThan(50);
      }
    }
  });
});
