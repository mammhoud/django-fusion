...
2. `lms-fusion/backend/plugins/<app>/templates/` — plugin/app templates
3. `lms-fusion/backend/www/<app>/templates/` — www app templates
4. `lms-fusion/backend/assets/templates/` — project asset templates
...
1. `lms-fusion/backend/assets/static/` — project-specific compiled static files
2. `lms-fusion/backend/assets/media/` — project-specific uploaded media
...
4. **Source assets at project root** — design source files (SCSS, logos, fonts,
   images) live in `lms-fusion/assets/` (sibling of `backend/` and `frontend/`).
