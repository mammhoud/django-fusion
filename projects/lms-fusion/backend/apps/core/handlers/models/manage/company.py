...
class Organization(DefaultBase, ClusterableModel):
    ...
    class Meta:
        ...
        db_table = "handlers_companies" if "handlers" in __name__ else "shared_companies" if "shared" in __name__ else "companies"
        ordering = ["name"]
        indexes = [
...
class Department(DefaultBase, ClusterableModel):
    ...
    class Meta:
        ...
        db_table = "handlers_departments" if "handlers" in __name__ else "shared_departments" if "shared" in __name__ else "departments"
        ordering = ["company", "function", "name"]
        constraints = [
            models.UniqueConstraint(
                fields=["company", "name"],
                name=f"{'handlers' if 'handlers' in __name__ else 'shared' if 'shared' in __name__ else 'app'}_unique_dept_name_per_company"
            )
        ]
        indexes = [
            models.Index(fields=["slug"]),
    ...
    def get_employee_count(self):
        ...
        from apps.pages.accounts.models import TeamMembership
