...
Usage: Add ``"plugins.pages.wagtail_hooks"`` to your ``INSTALLED_APPS``
or ensure ``plugins.pages`` is loaded via ``AppConfig.ready()``.
"""
...
def fusion_before_serve(page, request, serve_args, serve_kwargs):
    ...
    from plugins.pages.models import FusionPage
