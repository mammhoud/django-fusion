...
class Migration(migrations.Migration):
    ...
    ``app_label = "shared"`` and ``db_table = "grep_background_task_log"``, but
    the ``shared`` app is not installed in loop-crm, so Django's migration
    machinery never generates the table. The Dramatiq task backend dual-writes
    this shared audit trail, so the table must be materialized by a product
    migration (same pattern as precis' domain app).
    """

    dependencies = [
        ('core', '0005_taskexecution'),
    ]
