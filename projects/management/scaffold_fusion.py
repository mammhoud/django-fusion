...
This script creates isolated directories under projects/lms-fusion and
projects/cms-fusion, copies the ctc-research backend/frontend structure,
...
SOURCE = ROOT / "projects" / "cms" / "ctc-research"
TARGETS = {
    "lms-fusion": "Fusion LMS",
...
def scaffold_backend(name: str, display_name: str, target: Path) -> None:
    ...
    if settings_file.exists():
        replace_in_file(settings_file, "ctc-research", name)
    ...
    for env_file in (backend_dst / "__main__.py", backend_dst / "server.py"):
        if env_file.exists():
            replace_in_file(env_file, "ctc-research", name)
...
def scaffold_branding_app(target: Path, display_name: str) -> None:
    ...
    context = branding / "context_processors.py"
    context.write_text(
        "import os\n"
