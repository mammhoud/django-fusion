...
is available at [`../../precis-ctc/.env.example`](../../precis-ctc/.env.example). Secret values belong in
the deployment secret store or an ignored `.env` file; never commit them.

### Compose env-file resolution (SECRET_KEY consistency)

`backend/docker-compose.yml` reads two env files, both optional:

| Path (relative to `backend/`) | Purpose |
|---|---|
| `../.env` | Project-local overrides (`projects/precis/precis-ctc/.env`) |
| `../../../../.env` | **Repo-root `.env`** carrying `DJANGO_SECRET_KEY` + DB/Redis secrets |

> ⚠️ **Gotcha:** a relative path like `../../../.env` from `backend/` resolves
> to `projects/.env` (which does not exist) and is silently skipped. Every
> container process then auto-generates its own random `SECRET_KEY`, so the
> website and worker/scheduler sign sessions with *different* keys — token
> validation (e.g. the register → create-password email flow) fails with 400
> even though each process works in isolation. Keep the path at `../../../../.env`
> and confirm `DJANGO_SECRET_KEY` resolves identically in every container:
>
> ```bash
> docker exec precis-ctc-worker printenv DJANGO_SECRET_KEY | sha256sum
> docker exec precis-ctc-website printenv DJANGO_SECRET_KEY | sha256sum
> ```
...
| `DJANGO_SECRET_KEY` | secret | yes | **Must be shared across all backend processes** (website + worker + scheduler). Resolved from the repo-root `.env` — see the env-file note below |
| `SITE_URL` | `https://ctc-research.com` | yes | Absolute public origin used to build confirmation/verification links |
...
### Language persistence and localized Wagtail content

The canonical language settings are `LANGUAGE_CODE`, `LANGUAGES`,
`LANGUAGE_SESSION_KEY`, and the `LANGUAGE_COOKIE_*` values. The CTC selector
writes the chosen code to `sessionStorage`/`localStorage`, then POSTs to
`/i18n/setlang/`. The CTC endpoint stores the same code in the Django session
and returns the configured language cookie (`django_language` by default).

API/page requests resolve language in this order: explicit `?lang=`, Django
session, configured language cookie, request middleware language, then
`LANGUAGE_CODE`. Wagtail page APIs filter the published locale tree and return
`language` plus `available_languages`; they do not substitute frontend copy.
The `/apis/content/languages/` response also establishes the CSRF cookie needed
by the selector before its POST.

Useful checks:

```bash
curl -c /tmp/ctc-language.cookies https://ctc-research.com/apis/content/languages/
curl 'https://ctc-research.com/apis/pages/faq/?lang=ar'
```

For the Astro static shell, changing language appends `?lang=<code>` and updates
the document `lang`/`dir` attributes. The authoritative translated content
road remains the Wagtail API/HTMX response; rebuild the shell when changing
build-time `PUBLIC_CONTENT_LANGUAGE`.
