...
class EmailQueueManager:
    ...
    def _queue_with_django_q(self, log, recipient, subject, template_name, context):
        ...
        task_id = async_task(
            'apps.domain.services.email.tasks.send_email_task',
            log.id,
            recipient,
            subject,
            template_name,
            context,
            hook='apps.domain.services.email.tasks.email_sent_hook',
            group='email',
    ...
    def _queue_with_celery(self, log, recipient, subject, template_name, context):
        ...
        try:
            from apps.domain.services.email.tasks import send_email_task
    ...
    def schedule_periodic_task(
        ...
        Args:
            name: Task name
            func: Function path (e.g., 'apps.domain.services.email.tasks.check_registrations_task')
