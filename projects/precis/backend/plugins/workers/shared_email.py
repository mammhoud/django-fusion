...
across all registered websites. Each actor
...
Queue: All email actors run on the shared ``email`` queue.
"""
...
EMAIL_SERVICE_PATHS = [
    "apps.handlers.services.email.service.EmailService",
    "apps.pages.accounts.management.services.email.service.EmailService",
    "apps.domain.services.email.email_service.EmailService",
]


@task(queue="email", actor_name="shared.email.send")
def send_email_task(
    to: str,
    subject: str,
    template: str,
...
@task(queue="email", actor_name="shared.email.bulk")
...
@task(queue="email", actor_name="shared.email.raw")
def send_email_raw(
    subject: str,
    recipients: list[str],
    html_content: str,
