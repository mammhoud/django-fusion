...
Provides two key functions used by the Precis-local Dramatiq and Celery
actors in ``configs.tools.worker``:

1. :func:`configure_django_for_website` — Configures Django's
   environment (``DJANGO_SETTINGS_MODULE``, ``sys.path``) for a
   the Precis website before executing a task. Cross-project workers must
   provide their own configuration package and database boundary.
...
def configure_django_for_website(website: str | None = None) -> str:
    ...
    Args:
        website: Optional Precis website slug (for example ``"lms-fusion"``).
            Falls back to ``DJANGO_SITE``, ``DJANGO_WEBSITE``, or ``WEBSITE``.
    ...
    for path in (site_module.site_dir_for(selected), site_module.PROJECT_DIR, site_module.WORKSPACE_DIR):
        path_text = str(path)
