"""Celery tasks for the Precis/LMS worker.

This module defines Celery tasks that run in the Precis-owned
``shared-scheduler`` container and are consumed by the Precis/LMS worker.
Other products must use their own project-owned worker and database boundary.
The primary task is a periodic
heartbeat that verifies the Celery beat scheduler is alive and
functioning.

Auto-discovered by ``configs.tools.worker.celery`` via
...
- The Precis/LMS worker consumes the Precis queue; CTC Research and VResume
  queues are intentionally not configured in the Precis-owned task Compose.
...
@shared_task(
    name="configs.tools.worker.tasks.heartbeat",
    # Keep the schedule on the Precis/LMS queue. Beat will route by the
    # PeriodicTask row as well, but this decorator default is documentation-
    # as-code and prevents an accidental cross-project queue.
    queue="lms-fusion",
    bind=True,
    max_retries=0,
    acks_late=False,
...
def heartbeat(self):  # noqa: D401 — Celery ``bind=True`` signature
    ...
    The heartbeat runs on the ``lms-fusion`` queue and is consumed by the
    Precis/LMS Celery worker. The ``shared-worker`` Dramatiq container ignores
    this task entirely.
