"""Quiz system models (Precis parity).

Ported from Precis ``apps/learning/models/quiz.py`` — Quiz, QuizQuestion,
QuizChoice, QuizAttempt and QuizAnswer. Adapted to the landing-fusion app:
the lesson relation targets ``learning.Lesson`` (no legacy ``lms`` label).
"""
...
class QuizChoice(models.Model):
    ...
    choice_text = models.CharField(max_length=500, verbose_name=_("Choice Text"))
...
class QuizAttempt(models.Model):
    ...
    attempt_id = models.CharField(max_length=50, unique=True, editable=False)
    ...
    points_earned = models.PositiveIntegerField(
        default=0, verbose_name=_("Points Earned")
    )

    points_possible = models.PositiveIntegerField(
        default=0, verbose_name=_("Points Possible")
    )

    passed = models.BooleanField(
        null=True, blank=True, verbose_name=_("Passed")
    )
...
class QuizAnswer(models.Model):
    ...
    is_correct = models.BooleanField(
        null=True, blank=True, verbose_name=_("Is Correct")
    )

    points_awarded = models.PositiveIntegerField(
        default=0, verbose_name=_("Points Awarded")
    )
