...
this module supplies the precis-landing ``nav_items``, curated dropdown
...
NAV_ITEMS: tuple[tuple[str, str, bool], ...] = (
    ...
    # Benefits moved under Services (/services/benefits/) — no longer a
    # top-level nav item; the Services dropdown carries the link instead.
    ("blog", "Blog", True),
...
class LandingModule(Landing):
    ...
    def tree_children(self, current_path: str) -> dict[str, list[dict[str, Any]]]:
        ...
        excluded; delivery phases are consolidated under /about/services/ and
        so no longer appear as Services children. Returns ``{}`` (or a partial
        map) on error — never raises, so navigation keeps rendering.
        """
        ...
        try:
            ...
            for top in home.get_children().live().specific():
                ...
                for child in children_pages:
                    # Delivery phases are consolidated under /about/services/
                    # now, so they no longer appear as Services dropdown
                    # children (their /services/phases/* URLs redirect).
    ...
    def child_href(child) -> str:
        ...
        are consolidated under /about/services/ (their /services/phases/*
        URLs redirect), so any surviving PhasePage link points there.
        """
        from apps.pages.models import PhasePage
