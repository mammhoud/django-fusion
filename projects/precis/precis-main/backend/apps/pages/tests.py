...
class LandingPagesTestCase(TestCase):
    ...
    def test_all_pages_render(self):
        ...
        expected_hero = {
            ...
            "/services/benefits/": b"Built to ship as",
    ...
    def test_team_subpage_renders(self):
        ...
        self.assertEqual(members[0]["name"], "Mahmoud")
        self.assertEqual(members[0]["role"], "Founder & CEO")
    ...
    def test_products_and_features_carry_full_document(self):
        ...
        for path in ("/products/", "/services/benefits/"):
            with self.subTest(path=path):
                response = self.client.get(path)
                self.assertEqual(response.status_code, 200, path)
                for marker in (
    ...
    def test_page_fragment_api_streams_sse_without_fallback(self):
        """The single fragment source also streams over SSE when requested.

        ``Accept: text/event-stream`` returns a ``text/event-stream`` body with
        one ``fragment`` event carrying the same content region the HTMX road
        swaps in — backend HTML is the single source, no client fallback.
        """
        response = self.client.get(
            "/fragment/pages/about/", HTTP_ACCEPT="text/event-stream"
        )
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response["Content-Type"], "text/event-stream")
        body = b"".join(response.streaming_content).decode()
        self.assertIn("event: fragment", body)
        self.assertIn("data: ", body)
        # Complete component region (not a JSON blob, not an Astro shell).
    ...
    def test_pricing_page_links_edition_previews(self):
        ...
        # Formints 4 + Precis LMS 2 (Solo, Business) + Loop CRM 2 + Syntara 2
        # + vResume 2 + CTC 2 = 14. ceptor-ai is hidden and stays out.
        self.assertGreaterEqual(response.content.count(b"Preview this edition"), 14)
        for url in (
    ...
    def test_arabic_subpages_are_seeded_and_exposed(self):
        ...
        # Delivery phases + prompts are consolidated under /about/services/ —
        # their legacy URLs redirect permanently there.
        phase_html = self.client.get("/services/phases/discover/?lang=ar")
        self.assertEqual(phase_html.status_code, 301)
        self.assertEqual(phase_html["Location"], "/about/services/")

        prompt_html = self.client.get(
            "/services/phases/discover/prompts/shape-the-brief/?lang=ar"
        )
        self.assertEqual(prompt_html.status_code, 301)
        self.assertEqual(prompt_html["Location"], "/about/services/")
    ...
    def test_navigation_children_feed_dropdowns(self):
        ...
        # About → curated children (services/team/founder/startup routes).
        about = by_href["/about/"]
        self.assertEqual(
            [c["href"] for c in about["children"]],
            ["/about/services/", "/about/team/", "/about/founder/", "/about/startup/"],
        )
        self.assertEqual(about["children"][0]["label"], "Services")
        ...
        # Services → benefits subpage (phases consolidated under About → Services).
        services = by_href["/services/"]
        self.assertEqual([c["href"] for c in services["children"]], ["/services/benefits/"])
    ...
    def test_ctc_research_is_seeded_as_a_benefit_led_product(self):
        ...
        response = self.client.get("/products/precis-ctc/")
        self.assertEqual(response.status_code, 200)
        for marker in (
            b"CTC Research",
            b"Clearer evidence",
        ...
        page = self.client.get("/apis/pages/precis-ctc/").json()
        ...
        self.assertIn("/products/precis-ctc/", [child["href"] for child in products["children"]])

        brand = self.client.get("/apis/brand/").json()
        ctc_board = next(board for board in brand["boards"] if board["slug"] == "precis-ctc")
    ...
    def test_precis_lms_has_solo_and_business_only(self):
        ...
        self.assertEqual(
            business["features"],
            ["Everything in Solo", "Custom branding", "API access"],
        )
        self.assertEqual(business["price"], "$99")
    ...
    def test_seed_clears_legacy_product_snippets_and_refreshes_catalog_faq(self):
        ...
        # The Products document carries the catalog FAQ (DEFAULT_FAQ_SECTIONS)
        # — stale names (django-bolt) never leak into the data road payload.
        catalog = self.client.get("/apis/pages/products/")
        self.assertEqual(catalog.status_code, 200)
        self.assertNotIn(b"django-bolt", catalog.content)
        self.assertTrue(ProductsPage.objects.first().faq)
        ...
        # The stale FAQ is refreshed from DEFAULT_FAQ_SECTIONS — still present,
        # now without the removed library name.
        self.assertTrue(ProductsPage.objects.first().faq)
        self.assertNotIn(
            b"django-bolt",
            self.client.get("/apis/pages/products/").content,
    ...
    def test_formint_pos_renders_all_editions_with_pricing(self):
        ...
        response = self.client.get("/products/formint-pos/")
        self.assertEqual(response.status_code, 200)
        for marker in (b"Community", b"Standard", b"Pro", b"Cloud", b"$0", b"$119", b"$79", b"SQLite", b"Rust", b"See it in motion"):
            self.assertIn(marker, response.content)
        ...
        self.assertIn(b"$79", response.content)
    ...
    def test_catalog_only_products_stay_out_of_home_and_product_dropdown(self):
        ...
        # Syntara is a live product — visible on every catalog surface.
        self.assertIn("cypercloud", catalog_slugs)
        self.assertIn("cypercloud", pricing_slugs)
        self.assertIn("cypercloud", home_slugs)
        self.assertIn("cypercloud", dropdown_slugs)
        self.assertFalse(ProductPage.objects.get(slug="cypercloud").hidden)
        self.assertTrue(ProductPage.objects.get(slug="cypercloud").show_on_home)
    ...
    def test_products_carries_merged_catalog(self):
        ...
        # Syntara and CTC are now live product cards in the merged catalog.
        self.assertIn(b"/products/cypercloud/", response.content)
        self.assertIn(b"Syntara", response.content)
    ...
    def test_brand_page_renders_identity_boards(self):
        ...
        for data_brand in (b"data-brand=\"formints\"", b"data-brand=\"precis\"", b"data-brand=\"loop\"", b"data-brand=\"vresume\"", b"data-brand=\"syntara\"", b"data-brand=\"precis-ctc\""):
    ...
    def test_brand_api_and_modal_triggers(self):
        ...
        self.assertEqual(slugs, ["formint-pos", "lms", "cms", "cypercloud", "vresume", "precis-ctc"])
        board = boards[0]
        ...
        for slug in ("formint-pos", "lms", "cms", "cypercloud", "vresume", "precis-ctc"):
    ...
    def test_pricing_renders_product_tabs_and_faq(self):
        ...
        self.assertEqual(
            slugs, ["formint-pos", "lms", "cms", "cypercloud", "vresume", "precis-ctc"], slugs
        )
