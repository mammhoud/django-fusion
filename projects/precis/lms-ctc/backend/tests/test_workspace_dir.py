...
(``<repo>/projects/precis/ctc-research/backend/configs/settings/conf.py`` is seven levels
...
class TestWorkspaceDirHostLayout:
    ...
    def test_resolves_to_dir_owning_libs(self, monkeypatch, tmp_path):
        ...
        (tmp_path / "libs").mkdir()
        fake = _fake_conf(
            monkeypatch, tmp_path, "projects/precis/main/backend/configs/settings/conf.py"
        )
