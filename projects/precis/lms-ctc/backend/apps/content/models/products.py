"""
Precis Product snippet — editor-managed product catalog (landing-fusion parity).

Mirrors landing-fusion's ``apps/content/models/products.py`` and this app's
``course.py`` snippet: products are Wagtail snippets carrying the same
language + unified-currency contract as courses, so both websites share one
organization and one pricing architecture.

Serves: GET /api/products (?language=) — the catalog-of-record for language
filtering and pricing; the Wagtail product child pages remain the rich
product documents.
"""
...
class Product(index.Indexed, models.Model):
    ...
    class Meta:
        # The content AppConfig registers as ``label="pages"`` in precis, so
        # models must use that label (legacy ``content`` tables live in the
        # old cms-fusion DB and are not part of the precis schema).
        app_label = "pages"
    ...
    @classmethod
    def by_slug(cls):
        """Published products keyed by slug — used to enrich product cards."""
        return {
            item.slug: item
            for item in cls.objects.filter(is_published=True)
        }
