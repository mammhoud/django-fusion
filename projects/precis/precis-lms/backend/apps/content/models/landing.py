...
class LandingPage(BasePage):
    ...
    Rebases the landing slice onto lms's canonical ``BasePage``
    (WagtailPageMixin) so the ported marketing pages share the same
    render pipeline, ``show_page_at_footer`` control and context building
    as the rest of the LMS tree. Adds the shared hero + CTA sections plus
    SEO-friendly search fields.
    """
...
class ProductsPage(SectionStackMixin, LandingPage):
    ...
    def get_product_cards(self, for_home: bool = False) -> list[dict]:
        ...
        return [
            ...
            if isinstance(child.specific, ProductPage)
