...
`precis-lms` site. The Astro frontend in `../frontend` consumes the additive
...
migrations or rename the existing `precis-lms` apps. The LMS migration graph
...
```bash
cd projects
uv run python precis-lms/manage.py showmigrations --plan
uv run python precis-lms/manage.py check
```
