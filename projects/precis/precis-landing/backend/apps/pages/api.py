...
Mirrors precis-lms's /apis/ pattern but trimmed to landing needs.
"""
...
def _navigation_from_wagtail_tree(request):
    ...
    if not nav_items:
        nav_items = [
            ...
            {"label": "Services", "href": "/services/", "active": False, "children": []},
            {"label": "Benefits", "href": "/services/benefits/", "active": False, "children": []},
...
def page_fragment_api(request, slug="home"):
    ...
    fragment_context = {
        ...
        # Mirror the full-page context so the fragment is a complete, drop-in
        # replacement: the About-section subnav (team/founder/startup) and the
        # product stack strip render identically on both roads.
        "about_subnav_current": localization_view._get_about_subnav_current(specific),
        "show_stack_strip": specific.__class__.__name__ in ("ProductsPage", "ProductPage"),
    ...
    # SSE transport: when the client advertises ``Accept: text/event-stream``,
    # stream the rendered fragment as a single ``fragment`` event — the same
    # complete component the HTMX road swaps in. The minimal Astro shell
    # listens for this event instead of holding a static fallback.
    if "text/event-stream" in request.META.get("HTTP_ACCEPT", ""):
        ...
        streamer = SSHTMXFragmentStreamer(
            request, "pages/fragments/page.html", fragment_context
        )
...
def contact_submit_api(request):
    ...
    a ``ContactSubmission`` snippet (the archived precis-ctc / Precis
    ...
    # model mirrors the archived precis-ctc / Precis ContactSubmission.
    try:
