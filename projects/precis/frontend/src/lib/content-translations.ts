...
const CONTENT: Record<string, Partial<PageData>> = {
  ...
  projects: {
    ...
    projects: [
      ...
      { name: 'Precis', edition: 'product', category: 'learning', path: 'projects/precis',      description: 'Medical research learning platform with a seven-language page tree.', features: [{ label: 'Course catalog + enrollments', shared: false }, { label: 'Seven-language locales', shared: true }] },
      { name: 'Loop', edition: 'product', category: 'content', path: 'projects/landing-fusion', description: 'Content-driven websites built from Wagtail blocks.', features: [{ label: 'Wagtail StreamField blocks', shared: true }, { label: 'HTMX forms', shared: true }] },
    ],
    cta: {
      title: 'Ship the document, not the app',
      subtitle: 'One HTTP response, finished HTML.',
