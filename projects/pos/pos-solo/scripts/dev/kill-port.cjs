 ...
 * Default ports: 1420 (Vite) and 8766 (sidecar).
 * Used before `vite` / `pnpm dev` to prevent "Port X is already in use" errors
 * from leftover Vite, Tauri dev server, or Robyn sidecar processes.
 *
 * Usage:
 *   node scripts/dev/kill-port.cjs              # kills ports 1420, 8766
...
// ── Resolve ports ────────────────────────────────────────────────────
// If CLI args given, use those; else if PORT env var, use that; else defaults
