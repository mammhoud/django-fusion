/**
 * End-to-end test: full fragment-rendering flow (POS Solo mirror).
 *
 * Mirrors the pos-full e2e test exactly — identical scenarios, assertions,
 * and fragment HTML. Tests the complete chain:
 *
...
describe('E2E: Full fragment-rendering flow (POS Solo)', () => {
  beforeEach(async () => {
    vi.clearAllMocks();
  ...
  describe('3. Fragment failure — error → fallback to data mode', () => {
    it('falls back to data mode when fragment fetch fails', async () => {
      vi.mocked(fetch).mockResolvedValueOnce({
        ok: true,
        json: () => Promise.resolve(HEALTH_CHECK_TRUE),
      } as Response);
