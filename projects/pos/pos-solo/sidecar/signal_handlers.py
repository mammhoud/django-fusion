"""
POS Solo — shared signal handlers (logging, webhooks, audit persistence).

Provides @receiver handlers for every signal:
  1. Logging — structured log output
  2. Webhook — HTTP POST callbacks
  3. Audit — persistent SignalEvent records
...
class _WebhookUrls(dict):
    """Dict subclass that reads webhook URLs from env vars on each access."""
...
def _send_webhook(signal_name: str, payload: dict) -> tuple[str, str]:
    """Send a synchronous webhook POST to the configured URL."""
...
def _log_signal(signal_name: str, action: str, node_id: str = "", details: str = "") -> None:
    ...
    logger.info(
        ...
        signal_name, action, node_id or "(global)", details,
    )
