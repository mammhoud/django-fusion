"""
Approval workflow route handlers - approve, reject, stats, pending queue.
"""
...
def register_approval_routes(app):
    ...
    async def approve_change(request, pk: int):
        ...
        result = await S.sync_engine.approve_changes(
            [pk],
            reviewer=body.get("reviewer", ""),
            notes=body.get("notes", ""),
        )
        await S._broadcast_config_event("approval_approved", "__approval__", {
            "approval_id": pk, "reviewer": body.get("reviewer", ""),
            "result": result.message,
        })
        if S.fire_config_changed:
            S.fire_config_changed(
                node_id="__approval__",
                config_key=f"approval_{pk}",
                action="approved",
                new_value={"reviewer": body.get("reviewer", ""), "notes": body.get("notes", "")},
            )
    ...
    async def reject_change(request, pk: int):
        body = request.json() or {}
        result = await S.sync_engine.reject_changes(
            [pk],
            reviewer=body.get("reviewer", ""),
            notes=body.get("notes", ""),
        )
        await S._broadcast_config_event("approval_rejected", "__approval__", {
            "approval_id": pk, "reviewer": body.get("reviewer", ""),
        ...
        if S.fire_config_changed:
            S.fire_config_changed(
                ...
                config_key=f"approval_{pk}",
                action="rejected",
                new_value={"reviewer": body.get("reviewer", ""), "reason": body.get("notes", "")},
            )
        return jsonify(result.__dict__)

    @app.get("/approvals/stats")
    async def approval_stats(request):
        stats = await S.sync_engine.get_approval_stats()
    ...
    async def pending_approvals(request):
        ...
        result = await S.sync_engine.get_pending_approvals(
