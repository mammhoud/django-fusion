 ...
 *
 * Unlike the LMS version (which uses synchronous ``sessionStorage`` via
 * ``FusionDecoder``), this POS version uses the async ``fusionStore``
 * adapter that supports Tauri's ``@tauri-apps/plugin-store`` as a
 * persistence backend, falling back to an in-memory map.
 *
 * The preference is initialised on mount from the sidecar's
 * ``/fusion/health`` endpoint (which was already hit in ``main.tsx``
 * at startup).
 *
 * Usage in a layout::
 *
