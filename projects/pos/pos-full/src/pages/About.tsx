...
export default function About() {
  ...
  return (
    ...
    >
        ...
        {/* Main Content wrapped with FusionPage for fragment-rendering support */}
        <FusionPage standalone data={{ version: packageJson.version }}>
          {(data, fallback) => (
            <motion.div
              ...
              <div className="card--glass rounded-2xl p-6 md:p-8 transition-colors duration-300">
                ...
                <form onSubmit={handleSupportSubmit} className="space-y-5">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div>
                      ...
                      <input type="text" name="name" value={supportForm.name} onChange={handleSupportChange}
                        className={`w-full px-4 py-2.5 rounded-lg bg-white/50 dark:bg-white/5 border text-slate-900 dark:text-white focus:outline-none transition-colors ${formErrors.name ? 'border-red-500 focus:border-red-400' : 'border-slate-300 dark:border-gray-600 focus:border-teal-400'}`}
                        disabled={isSubmitting} placeholder={t('support.namePlaceholder')} />
                    ...
                    <div>
                      ...
                      <input type="email" name="email" value={supportForm.email} onChange={handleSupportChange}
                        className={`w-full px-4 py-2.5 rounded-lg bg-white/50 dark:bg-white/5 border text-slate-900 dark:text-white focus:outline-none transition-colors ${formErrors.email ? 'border-red-500 focus:border-red-400' : 'border-slate-300 dark:border-gray-600 focus:border-teal-400'}`}
                        disabled={isSubmitting} placeholder={t('support.emailPlaceholder')} />
                  ...
                  <div>
                    ...
                    <input type="text" name="subject" value={supportForm.subject} onChange={handleSupportChange}
                      className={`w-full px-4 py-2.5 rounded-lg bg-white/50 dark:bg-white/5 border text-slate-900 dark:text-white focus:outline-none transition-colors ${formErrors.subject ? 'border-red-500 focus:border-red-400' : 'border-slate-300 dark:border-gray-600 focus:border-teal-400'}`}
                      disabled={isSubmitting} placeholder={t('support.subjectPlaceholder')} />
                  ...
                  <div>
                    ...
                    <textarea name="message" value={supportForm.message} onChange={handleSupportChange} rows={6}
                      className={`w-full px-4 py-2.5 rounded-lg bg-white/50 dark:bg-white/5 border text-slate-900 dark:text-white focus:outline-none transition-colors resize-none placeholder:text-slate-400 dark:placeholder:text-gray-400 ${formErrors.message ? 'border-red-500 focus:border-red-400' : 'border-slate-300 dark:border-gray-600 focus:border-teal-400'}`}
                      disabled={isSubmitting} placeholder={t('support.messagePlaceholder')} />
                    {formErrors.message && <p className="text-red-400 text-sm mt-1">{formErrors.message}</p>}
                  </div>
                  <motion.button type="submit"
                    className="w-full py-3 bg-linear-to-r from-teal-400 to-purple-400 text-white rounded-xl font-semibold transition-all duration-200 flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                    disabled={isSubmitting}>
                    {isSubmitting ? (
                      <><motion.div animate={{ rotate: 360 }} transition={{ duration: 1, repeat: Infinity, ease: "linear" }} className="w-5 h-5 border-2 border-white border-t-transparent rounded-full" />{t('about.sending')}</>
                    ) : (
                      <><FaPaperPlane className="text-lg" />{t('about.sendMessage')}</>
                    )}
                  </motion.button>
                </form>
                <div className="mt-6 pt-6 border-t border-slate-200 dark:border-white/10">
                  ...
                  <div className="mt-3 space-y-2 text-sm text-slate-600 dark:text-gray-300">
                    <p>🌐 {t('about.visitWebsite')}{' '}
                      <a href="https://structa.cloud" target="_blank" rel="noopener noreferrer"
      ...
      <AnimatePresence>
        {submitStatus === 'success' && (
          <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 50 }}
            className="fixed bottom-8 left-1/2 -translate-x-1/2 bg-teal-500 text-white px-6 py-3 rounded-xl flex items-center gap-2 shadow-lg z-50">
            <FaCheck className="text-xl" /> {t('about.successToast')}
          </motion.div>
        )}
        {submitStatus === 'error' && (
          <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 50 }}
            className="fixed bottom-8 left-1/2 -translate-x-1/2 bg-red-500 text-white px-6 py-3 rounded-xl flex items-center gap-2 max-w-md shadow-lg z-50">
            <FaExclamationTriangle className="text-xl" /> <span>{errorMessage}</span>
          </motion.div>
        )}
      </AnimatePresence>
    </PageLayout>
