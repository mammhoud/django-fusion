 ...
 *
 *     // In components
 *     const shouldUseFragments = fusionStore.getSessionPreference();
 */
...
/**
 * Attempt to load the Tauri store plugin; falls back to sessionStorage.
 */
...
async function getAdapter(): Promise<StoreAdapter> {
  ...
  // Fall back to in-memory map (sessionStorage may not be available in Tauri)
...
export const fusionStore = {
  /** Initialise the session preference (called after health check or fragment response). */
  async initSession(value: boolean): Promise<void> {
    const adapter = await getAdapter();
    await adapter.setItem(SESSION_KEY, String(value));
  },

  /** Return the cached preference, or undefined if not set. */
  ...
  /**
   * Fetch the rendering preference from the sidecar's ``/fusion/health``
   * endpoint and cache it. Returns the preference.
   */
