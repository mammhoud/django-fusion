//! Server lifecycle management for the POS Python/Sanic backend.
//!
//! The server is a PyInstaller-bundled executable located at
//!   `<app resources>/binaries/pos-server-<target-triple>`
//! Tauri's ExternalBin mechanism appends the target triple automatically.
//!
//! The server is started once on app launch (via `setup`) and
//! kept alive in a global `OnceCell`. The frontend can also call
//! `start_server` / `stop_server` / `server_status` via `invoke()`.
//!
//! Communication between the React frontend and the server happens
...
//! spawning `python3 server/server.py` from the project root.
...
/// Holds the running server process. `None` when not started.
static SERVER_PROCESS: OnceCell<Mutex<Option<CommandChild>>> = OnceCell::new();
...
/// Resolve the path to the SQLite DB file that the server should read.
...
/// Start the Python server. Idempotent — calling it while it is already
...
pub fn start_server(app: AppHandle) -> Result<String, String> {
    ...
    // Try the bundled server first; fall back to python in development.
    let (mut rx, child) = if let Ok(cmd) = app.shell().sidecar("pos-server") {
        cmd.args(args.clone()).spawn()
            .map_err(|e| format!("server spawn error: {e}"))?
    } else {
        eprintln!("[server] bundled binary not found, falling back to python");
        ...
        app.shell()
            ...
            .args([&["server/server.py"], &args[..]].concat())
            .spawn()
            .map_err(|e| format!("{python_cmd} server spawn error: {e}"))?
    };
                ...
                match event {
                    CommandEvent::Stdout(line) => {
                        let _ = String::from_utf8(line)
                            .map(|s| eprintln!("[server stdout] {s}"));
                    }
                    CommandEvent::Stderr(line) => {
                        let _ = String::from_utf8(line)
                            .map(|s| eprintln!("[server stderr] {s}"));
                    }
    ...
    // channel will fill and block the server process.
...
/// Return whether the server process is currently running.
#[tauri::command]
