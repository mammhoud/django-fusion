"""
Formint — Fusion render-mode contract (mirrors precis-landing's /apis/ helpers).

Dual-mode content delivery, identical to ``projects/precis/precis-landing/backend/apps/pages/api.py``:
...
Endpoints exposed here:
    ...
    GET /api/v1/navigation/    — nav items from FormintModule (source of truth)
...
# ── Effective render-first (mirrors precis-landing's helper) ───────────────
...
def get_effective_render_first(request: HttpRequest | None = None) -> bool:
    ...
    the ``FUSION_RENDER_FIRST`` setting.  The session is only consulted when
    an operator explicitly stored a preference — never auto-seeded — so a
    data-API default stays reachable for browser traffic.
...
def navigation_payload(request: HttpRequest | None = None) -> dict[str, Any]:
    """Build the hierarchical nav payload from FormintModule.
    ...
    return formint_module.get_navigation_context(request)
...
def navigation_api(request: HttpRequest) -> JsonResponse:
    ...
    Uses ``FormintModule.get_navigation_context()`` as the single source of
