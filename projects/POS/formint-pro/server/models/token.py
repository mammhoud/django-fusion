"""POS Full Edition — DeviceToken (thin wrapper over BaseDeviceToken).
...
class DeviceToken(BaseDeviceToken):
    """POS Full edition device token — inherits all behaviour from BaseDeviceToken.

    Keeps only server-specific indexes.  ``token_hash`` is already
