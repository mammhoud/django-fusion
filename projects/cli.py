...
SITES = {
    ...
    "shared": {
        ...
        # Ad-hoc invocations of `python projects/cli.py ... --site www`
        # (or via alias `--site shared`) trip the explicit-empty guard
        # rendered by `SiteCLI.site_env()` and raise a clear SystemExit
        # instead of silently landing on the wrong DB.
        "db_name": None,
    },
}
...
class SiteCLI:
    ...
    def site_env(self) -> Dict[str, str]:
        ...
        # worker, but ad-hoc `python projects/cli.py ... --site shared` runs
