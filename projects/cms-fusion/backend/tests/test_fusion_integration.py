"""
E2E integration tests for the django-fusion fragment-rendering flow on CTC Research.

Verifies the full backend → API contract that the frontend FusionPage,
FusionProxy, and FusionMiddleware components depend on:

1. ``/api/fusion/health`` — health-check response envelope
2. ``/api/pages/<slug>/fragment/`` — fragment-pointer contract
3. ``/api/pages/<slug>/data/`` — unified data/fragment endpoint
...
# ═══════════════════════════════════════════════════════════════════════
# 1. Health endpoint — /api/fusion/health
# ═══════════════════════════════════════════════════════════════════════

class TestFusionHealthEndpoint:
    """Contract: ``/api/fusion/health`` returns the canonical envelope."""

    def test_returns_fusion_envelope(self):
        """The response has the ``{status, message, data}`` envelope."""
        request = _with_session(RequestFactory().get("/api/fusion/health"))
...
# ═══════════════════════════════════════════════════════════════════════
# 2. Fragment pointer — /api/pages/<slug>/fragment/
# ═══════════════════════════════════════════════════════════════════════
...
class TestFragmentPointerContract:
    ...
    def _get_pointer(self, slug: str = "home"):
        request = RequestFactory().get(f"/api/pages/{slug}/fragment/")
    ...
    def test_envelope_shape_has_status_message_data(self):
        ...
        request = RequestFactory().get("/api/pages/home/fragment/")
    ...
    def test_not_found_returns_404_envelope(self):
        ...
        request = RequestFactory().get("/api/pages/unknown/fragment/")
    ...
    def test_fragment_name_follows_convention(self):
        ...
        assert self._get_pointer("about")["fragment_name"] == "pages.about"
        ...
        assert self._get_pointer("services")["fragment_name"] == "pages.services"
        assert self._get_pointer("team")["fragment_name"] == "pages.team"


# ═══════════════════════════════════════════════════════════════════════
# 3. Page data endpoint — /api/pages/<slug>/data/
# ═══════════════════════════════════════════════════════════════════════

class TestPageDataContract:
    """Contract: ``/api/pages/<slug>/data/`` response is consumable by frontend."""

    def test_data_mode_returns_page_data_response(self):
        """In data mode, the inner data matches ``PageDataResponse``."""
        request = RequestFactory().get("/api/pages/home/data/")
    ...
    def test_encoded_field_matches_codec_format(self):
        ...
        request = RequestFactory().get("/api/pages/about/data/")
        response = page_data(request, "about")
        payload = json.loads(response.content)
    ...
    def test_encoded_field_decodeable_by_frontend_decoder(self):
        ...
        request = RequestFactory().get("/api/pages/faq/data/")
    ...
    def test_all_pages_produce_decodeable_encoded_field(self):
        ...
        for slug in STATIC_PAGES:
            request = RequestFactory().get(f"/api/pages/{slug}/data/")
    ...
    def test_not_found_returns_404_with_error(self):
        ...
        request = RequestFactory().get("/api/pages/unknown/data/")
...
class TestFusionCodecRoundtrip:
    ...
    def test_decode_page_data_with_frontend_api(self):
        ...
        assert decoded["title"] == "CTC Research — Advancing Clinical Trials"
        assert "blocks" in decoded
    ...
    def test_fusion_response_produces_frontend_compatible_payload(self):
        ...
        pointer = fusion_response(
            "pages.home",
            extra={"page_slug": "home", "title": "CTC Research"},
        )
        ...
        assert pointer["title"] == "CTC Research"
...
class TestEndToEndFlow:
    ...
    def test_frontend_can_consume_fragment_pointer(self):
        ...
        1. Fetch /api/pages/<slug>/fragment/
        ...
        #   <FusionProxy fragmentUrl={`/api/pages/home/data/?fusion_render_first=true`} />
        unified_url = "/api/pages/home/data/?fusion_render_first=true"
