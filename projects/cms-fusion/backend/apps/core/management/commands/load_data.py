...
The fixture uses natural keys for ContentType and User references, so
Django's loaddata resolves them automatically *provided* the referenced
records exist.  This command ensures prerequisites are met before loading:

    1. Sync content types for all installed apps
    2. Create admin superuser (referenced as ``['admin']`` by pages + subs)
    3. Create Wagtail Collections (``Root → Media → Main Photos (assets)``)
    4. Remove auto-created pages (except Root), sites, images so the
       fixture's hand-coded PKs (Root=1, Home=3, …) import without conflict
    5. Load ``dump-data.json``
    6. Create a default Wagtail Site pointing at the imported home page
...
Note: this command is designed for a *fresh* database.  On re-run the
clear step (4) deletes all existing fixture data and the fixture re-import
will fail on duplicate PKs.
"""
        ...
        if not dry_run:
            ...
            for app_config in apps.get_app_configs():
                try:
                    ...
                    # App tables may not exist yet (e.g. wagtailredirects).
                    # The transaction.atomic() ensures the connection is
                    # not left in a broken state. Skip gracefully —
                    # loaddata resolves content type natural keys at
                    # fixture-load time.
                    pass
