# services/enrollment_services.py
"""
Enhanced Enrollment Service with comprehensive operations for views.
"""
...
from ceptor_ai.models import Person as Profile
from ceptor_ai.services.infrastructure.base import BaseService
from ceptor_ai.services.infrastructure.token import TokenService

logger = logging.getLogger(__name__)

from plugins.lms.managers.enrollments import EnrollmentManager
from plugins.lms.models import Course, Enrollment
from plugins.lms.models.courses.progress import LessonProgress, ModuleProgress
from plugins.lms.models.courses.specification import Lesson

User = get_user_model()


class EnhancedEnrollmentService(BaseService):
    """
    Enhanced service layer for Enrollment operations with view-specific methods.
    """
    ...
    def _get_learning_analytics(cls, profile: Profile, time_period: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "completion_rate": (
                ...
                if metrics['total_courses_started'] else 0
            ),
        }
    ...
    def _get_active_courses_with_progress(cls, profile: Profile) -> List[Dict[str, Any]]:
        ...
        ).select_related('course', 'course__instructor').order_by('-last_accessed_at')
        ...
        try:
            ...
            if last_completed:
                ...
                next_lesson = Lesson.objects.filter(
                    ...
                    order__gt=last_completed.lesson.order
    ...
    def _get_learning_habits(cls, profile: Profile) -> Dict[str, Any]:
        ...
        sessions = Enrollment.objects.filter(
            student=profile,
        ...
        avg_session_minutes = sessions.aggregate(
            avg=Avg('total_time_spent_minutes') / Count('id')
        )['avg'] or 0
    ...
    def sync_enrollment_progress_from_external(
        ...
        user_id: int,
        course_id: int,
        external_progress_data: Dict[str, Any],
        token: str
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            return {
                ...
                "certificate_info": {
                    ...
                    "valid_until": enrollment.completed_at + timezone.timedelta(days=365),  # 1 year validity
