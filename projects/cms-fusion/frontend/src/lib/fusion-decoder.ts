 ...
 *
 *     // Decode a fragment-pointer payload
 *     const pointer = decoder.decodeFragmentPointer(response.data);
 *     // { component, fragment_name, fragment_url, fusion_render_first, ... }
 *
 *     // Check session-cached render-first preference
 *     if (decoder.shouldRenderFragmentFirst(pointer)) {
 *       // Show server-rendered fragment
 *     }
 */
...
export class FusionDecoder {
  ...
  decodeB64(payload: CodedPayload | string): unknown {
    ...
    let jsonStr: string;
    try {
      // Node.js Buffer-compatible (works in Next.js SSR too)
