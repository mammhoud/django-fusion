...
export default function CheckoutPage() {
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      <Link href="/cart" className="inline-flex items-center gap-2 text-gray-500 hover:text-[rgb(var(--fu-primary))] mb-8 transition-colors">
            ...
            {[
              ...
              <button key={method.value} type="button" onClick={() => setForm({...form, payment_method: method.value})}
                className={`px-4 py-3 rounded-lg border-2 text-sm font-medium transition-all ${
                  form.payment_method === method.value ? 'border-[rgb(var(--fu-primary))] bg-[rgb(var(--fu-primary))]/5 text-[rgb(var(--fu-primary-dark))]' : 'border-gray-200 text-gray-600 hover:border-gray-300'
                }`}>{method.label}</button>
            ))}
          </div>
        </div>
