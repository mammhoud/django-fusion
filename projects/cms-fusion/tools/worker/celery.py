...
2. Creates the Celery ``app`` instance configured from Django settings
   (``CELERY_`` namespace) with autodiscovery pointed at ``www.worker``.
...
# Shared tasks live in www.worker (Dramatiq actors). Celery is kept only for
# the django-celery-beat scheduler and any legacy/site-specific Celery tasks.
TASK_IMPORTS = ()
