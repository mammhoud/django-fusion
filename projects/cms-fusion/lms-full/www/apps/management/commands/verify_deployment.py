...
Ported from ``verify-demo-config.sh``.

Usage:
    uv run python manage.py verify_deployment
    uv run python manage.py verify_deployment --container lms-website
"""
...
_CONTAINER_MAP: dict[str, str] = {
    "docker": "lms-website",
    "demo": "lms-website",
    "production": "lms-website",
    "local": "lms-website",
}

_DEFAULT_CONTAINER = "lms-website"
...
def _check_traefik_config() -> list[tuple[bool | None, str]]:
    ...
    candidates = [
        Path("/root/site/compose/traefik/dynamic/lms.yml"),
        Path("/root/site/compose/traefik/dynamic/lms.yml"),
        Path("/root/site/compose/traefik/dynamic/lms.yml"),
    ]
    ...
    for name in ("lms.yml", "lms.yml", "lms.yml"):
    ...
    # Check for a lms domain
    if "structa.cloud" in content:
        results.append((True, "Domain configured: structa.cloud"))
    else:
        results.append((None, "structa.cloud domain not found in Traefik config"))
...
def _check_asset_health(base_url: str) -> list[tuple[bool | None, str]]:
    """Check 9 — Asset health via /health/assets/ endpoint."""
    results: list[tuple[bool | None, str]] = []

    try:
        import requests
        response = requests.get(f"{base_url}/health/assets/", timeout=10)
        ...
        if response.status_code == 200:
            ...
            if status == "ok":
                results.append((True, "Asset health check: OK"))
            elif status == "degraded":
                results.append((None, "Asset health check: DEGRADED"))
                # Add warnings
                for warning in data.get("warnings", []):
                    results.append((None, f"  Warning: {warning}"))
            else:
                results.append((False, f"Asset health check: {status.upper()}"))
                # Add errors
                for error in data.get("errors", []):
                    results.append((False, f"  Error: {error}"))

            # Report on individual checks
            checks = data.get("checks", {})
            if "static_root" in checks:
                check_status = checks["static_root"].get("status")
                if check_status == "ok":
                    results.append((True, "  Static files: OK"))
                else:
                    results.append((False, "  Static files: FAILED"))

            if "media_root" in checks:
                check_status = checks["media_root"].get("status")
                if check_status == "ok":
                    results.append((True, "  Media files: OK"))
                elif check_status == "warning":
                    results.append((None, "  Media files: WARNING"))
                else:
                    results.append((False, "  Media files: FAILED"))

            if "webpack_bundles" in checks:
                check_status = checks["webpack_bundles"].get("status")
                asset_count = checks["webpack_bundles"].get("asset_count", 0)
                if check_status == "ok":
                    results.append((True, f"  Webpack bundles: OK ({asset_count} assets)"))
                elif check_status == "warning":
                    results.append((None, "  Webpack bundles: WARNING"))
                else:
                    results.append((False, "  Webpack bundles: FAILED"))

        elif response.status_code == 503:
            data = response.json()
            results.append((False, "Asset health check: UNHEALTHY"))
            for error in data.get("errors", []):
                results.append((False, f"  Error: {error}"))
        else:
            results.append((False, f"Asset health endpoint returned {response.status_code}"))

    except ImportError:
        results.append((None, "requests library not available, skipping asset health check"))
    except Exception as e:
        results.append((False, f"Failed to check asset health: {str(e)}"))
...
class Command(BaseCommand):
    ...
    def handle(self, *args: Any, **options: Any) -> None:  # noqa: C901
        ...
        project_root = Path(__file__).resolve().parents[4]  # lms/
        ...
        # 9. Asset health
        # ------------------------------------------------------------------
        self.stdout.write("9. Checking Asset Health...")
        # Determine base URL based on container
        if "demo" in container:
            base_url = "http://localhost:8271"
        else:
            base_url = "http://localhost:8270"

        for ok, msg in _check_asset_health(base_url):
            if ok is True:
                self.stdout.write(_pass(msg))
            elif ok is None:
                self.stdout.write(_warn(msg))
