...
- Python/Robyn sidecar server (use [`formint`](../../formint/) for that)
- Zustand API stores (forge-pos uses `invoke()` directly)
- Django ORM models
- Cloud CRM sync

> Need a sidecar API + cloud sync? Use [`formint`](../../formint/) (the merged Pro package, formerly pos-full + pos-solo).
