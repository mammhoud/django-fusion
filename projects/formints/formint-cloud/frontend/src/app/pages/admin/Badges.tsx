...
/** Loyalty/reward badge — persisted locally until a backend table is added. */
...
const STORAGE_KEY = 'formint.rewardBadges.v1';
...
function loadBadges(): RewardBadge[] {
  try {
    const raw = localStorage.getItem(STORAGE_KEY);
    return raw ? (JSON.parse(raw) as RewardBadge[]) : [];
  } catch {
    return [];
  }
}

function persistBadges(badges: RewardBadge[]) {
...
export default function Badges() {
  ...
  const handleSubmit = () => {
    ...
    // Simulate an async write so the footer spinner + disabled states render.
    setTimeout(() => {
      try {
        const next = [...badges];
        if (editing) {
          const idx = next.findIndex((b) => b.id === editing.id);
          if (idx >= 0) {
            next[idx] = { ...next[idx], ...form, threshold, name: form.name.trim() };
          }
        } else {
          next.push({
            id: Date.now(),
        ...
        showError(`${t('common.error')}: ${err instanceof Error ? err.message : String(err)}`);
      } finally {
  ...
  const handleDelete = () => {
    if (!toDelete) return;
    const next = badges.filter((b) => b.id !== toDelete.id);
    persistBadges(next);
    setBadges(next);
    setToDelete(null);
