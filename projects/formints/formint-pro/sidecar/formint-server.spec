...
"""
PyInstaller spec for the Formint POS Robyn sidecar server.

Packages the merged sidecar (Robyn async server + Django ORM + Ninja +
...
Usage (via the sidecar Makefile):
    make sidecar-binary            # pyinstaller formint-server.spec --clean --noconfirm
    make sidecar-install-bin       # copy dist/formint-backend → ../src-tauri/binaries/
...
# break pathex resolution for top-level sidecar modules.
...
# so every INSTALLED_APPS package plus the sidecar's own modules is forced in.
hiddenimports = []
...
for _pkg in (
    ...
    "about",
    # Sidecar top-level modules (registered for their @receiver side effects)
    "ws_client",
    "ws_sync_signals",
    "sync_signals",
    "signal_handlers",
