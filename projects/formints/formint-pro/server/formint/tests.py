...
class FormintFusionRenderModeTests(TestCase):
    ...
    def test_api_navigation_returns_formint_module_items(self):
        response = self.client.get('/api/v1/navigation/')
        self.assertEqual(response.status_code, 200)
        data = response.json()['data']
        # FormintModule returns the nested {brand, modules} contract.
...
class FormintSyncApiTests(TestCase):
    """Cloud server sync API — push/receive/approve ledger endpoints."""
...
class FormintComponentsApiTests(TestCase):
    """django-fusion components served as data over the server API."""
