...
"""
PyInstaller spec for the Formint Django server binary.

Builds the Django management entry point (server.py) into a single
executable for Tauri server bundling (externalBin "binaries/formint-backend").
...
Usage:
    .venv/bin/pyinstaller server/formint-backend.spec --clean --noconfirm
...
a = Analysis(
    [os.path.join(_SPEC_DIR, "server.py")],
    pathex=[_SPEC_DIR],
    binaries=[],
    datas=datas,
