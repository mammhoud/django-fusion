          ...
          >
            ...
            <nav ref={navContainerRef} className="p-3 space-y-1 flex-1 overflow-y-auto">
              {visibleCategories.map((cat, i) => (
                <div
                  key={cat.id}
                  className={`nav-mask overflow-hidden ${revealed ? 'is-visible' : ''}`}
            ...
            {/* Footer */}
            <div
              className={`nav-mask overflow-hidden p-4 border-t border-base-300/50 mt-auto ${revealed ? 'is-visible' : ''}`}
              style={{ transitionDelay: `${200 + visibleCategories.length * 60}ms` }}
            >
              {isAuthRequired && (
                <button
                  onClick={handleLogout}
