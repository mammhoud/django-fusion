...
export default function Transactions() {
  ...
  // Refund & returns — Community capability: only `completed` sales can be
  // refunded, once. Local state flips the row to `refunded` immediately so
  // the chip is visible without waiting on the quiet reload.
  ...
  const handleRefund = async () => {
    ...
    try {
      ...
      setTransactions(prev =>
        prev.map(tx => (tx.id === refundTarget.id ? { ...tx, status: 'refunded' } : tx)),
      );
      setRefundTarget(null);
      showSuccess(t('transactions.refundSuccess'));
    } catch (error) {
  ...
  return (
    ...
    >
                      ...
                      {group.transactions.map(tx => (
                        <div key={tx.id} className="flex items-center justify-between p-2 rounded-lg bg-base-100/40">
                          <div className="flex items-center gap-3">
                            ...
                            {tx.status === 'refunded' && (
                              <span className="rounded-full border border-amber-300/60 bg-amber-100/80 px-2 py-0.5 text-[11px] font-semibold text-amber-800">
                          ...
                          <div className="flex items-center gap-2">
                            {tx.status === 'completed' && (
                              <button
                                ...
                                aria-label={t('transactions.refund')}
                                title={t('transactions.refund')}
                                className="text-amber-600 dark:text-amber-400 hover:text-amber-700 p-1.5 transition-all active:scale-[0.9]"
                              >
                                <span className="ri-refund-2-line ri-16px" />
                              </button>
                            )}
      ...
      />
      {/* Refund confirmation dialog — only reachable from a completed sale */}
      {refundTarget && (
        <Modal
          isOpen={!!refundTarget}
          ...
          footer={
            <>
              ...
              <button
                ...
                disabled={refunding}
                className="btn btn-warning flex-1 disabled:opacity-60"
              >
                <span className="ri-refund-2-line ri-16px" />
                {refunding ? t('transactions.refunding') : t('transactions.confirmRefund')}
              </button>
            </>
          }
        >
          <div className="text-center">
            <div className="flex justify-center mb-4">
              <div className="modal__body-icon modal__body-icon--warning">
                <span className="ri-refund-2-line text-4xl" />
              </div>
            </div>
            <h2 className="modal__title text-xl font-bold mb-2 text-center">
              {t('transactions.refundTitle')}
            </h2>
            <p className="text-base-content/70">
              {t('transactions.refundMessage', {
                id: refundTarget.id,
                amount: `${refundTarget.currency} ${refundTarget.total_amount.toFixed(2)}`,
              })}
            </p>
          </div>
        </Modal>
