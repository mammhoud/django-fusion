#!/usr/bin/env node
/**
 * POS Cloud (formintB) — Live end-to-end stack verification.
 *
...
const ROOT = resolve(dirname(fileURLToPath(import.meta.url)), '..', '..'); // formintB/
...
const logsDir = mkdtempSync(join(tmpdir(), 'pos-cloud-verify-'));
...
async function main() {
  console.log('── POS Cloud (formintB) live stack verification ────────────────');
