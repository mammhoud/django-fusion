"""
Configuration route handlers - device config, cloud links, master sync, config WebSocket.
"""
...
def register_config_routes(app):
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                resp = await client.get(
                    f"{link.cloud_url.rstrip('/')}/health",
                ...
                if resp.status_code == 200:
                    ...
                    def _mark_connected():
                        ...
                        link.last_connected_at = datetime.now(timezone.utc)
                        link.save(update_fields=["status", "last_connected_at", "updated_at"])
            ...
            def _mark_error():
                ...
                link.save(update_fields=["status", "status_message", "updated_at"])
            await _mark_error()
            return jsonify({"status": "error", "error": str(exc)})

    # ── Master device config sync ──
    ...
    async def sync_master_config(request, pk: int):
        ...
        if not managed_ids:
            return jsonify({"status": "ok", "synced": 0, "message": "No managed nodes to sync"})
        ...
        def _sync_nodes():
            ...
            for nid in managed_ids:
                for key, val in master.config.items():
                    S.DeviceConfig.objects.update_or_create(
                        node_id=nid, config_key=key,
                        defaults={"config_value": val, "category": "system"},
                    )
                    synced += 1
            S.NodeEvent.objects.create(
                node_id=master.device_id, event_type="sync_success",
                description=f"Master config synced to {len(managed_ids)} nodes ({synced} entries)",
                metadata={"device_id": master.device_id, "nodes": managed_ids, "entries": synced},
            )
            return {"synced": synced, "nodes": len(managed_ids)}

        result = await _sync_nodes()
        await S._broadcast_config_event("master_synced", master.device_id, {
            "master_device_id": master.device_id,
            "nodes_synced": result["nodes"],
            "entries_synced": result["synced"],
        })
        if S.fire_config_synced:
            S.fire_config_synced(
                source_device_id=master.device_id,
                target_device_ids=managed_ids,
                config_keys=list(master.config.keys()),
                status="success",
            )
        return jsonify({"status": "ok", **result})

    # ── WebSocket: Config event stream ──
    ...
    async def config_stream(websocket):
        ...
        try:
            await websocket.send_text(json.dumps({
                "type": "connected",
                "message": "Connected to configuration event stream",
                "client_id": client_id,
            ...
            while True:
                ...
                try:
                    ...
                    if "filter" in data:
                        ...
                        await websocket.send_text(json.dumps({
                            "type": "filter_updated",
                            "filter": S._config_ws_filters[client_id],
                        }))
