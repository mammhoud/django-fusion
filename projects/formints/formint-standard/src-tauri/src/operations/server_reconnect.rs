//! Server reconnect — health check, flush pending sync, HTTP proxy.
//!
//! When the Django server becomes available again after being offline,
//! the pending sync_queue rows are pushed to it so real DataToken rows
//! can be created by the Django server.

use tauri::Manager;

use crate::operations::sync_queue::{self, PendingSyncItem};

/// Default server base URL for the POS backend.
const SERVER_BASE_URL: &str = "http://127.0.0.1:8767";

/// Check whether the server is healthy by hitting its health endpoint.
/// Uses a short timeout (2 seconds) so the offline fallback is fast.
pub fn check_server_health() -> Result<bool, String> {
    let url = format!("{SERVER_BASE_URL}/health/");
    match ureq::get(&url)
...
/// Post a single sync item to the Django server's sync-proxy endpoint.
/// Called by both the dispatcher (on every write) and the flush loop.
pub fn post_sync_item_to_server(
    entity_type: &str,
    entity_id: &str,
    change_type: &str,
    payload_json: &str,
) -> Result<(), String> {
    let url = format!("{SERVER_BASE_URL}/sync-proxy/{entity_type}/");
...
/// Flush all pending sync_queue rows to the Django server.
///
/// Called when the server comes back online (detected via periodic
...
pub fn flush_pending_sync(app: tauri::AppHandle) -> Result<FlushResult, String> {
    ...
    for item in &batch {
        ...
        match post_sync_item_to_server(
            &item.entity_type,
            &item.entity_id,
            &item.change_type,
            &item.payload_json,
...
/// Tauri command: check server health and return status.
#[tauri::command]
pub fn check_server_health_cmd() -> Result<String, String> {
    match check_server_health() {
