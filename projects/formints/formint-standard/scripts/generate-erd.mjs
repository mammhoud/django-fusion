...
/**
 * (Re)generate the ERD artifacts.
 *
 * @param {{ outDir?: string, quiet?: boolean }} [opts]
 *   outDir — directory to write erd-data.js + erd.mmd into (default: docs/erd)
 *   quiet  — suppress the success log line
 * @returns {{ data, mmd, entities: number, relations: number, modules: number }}
 */
export function generate(opts = {}) {
  const outDir = opts.outDir ? resolve(ROOT, opts.outDir) : OUT_DIR;
  const outData = resolve(outDir, 'erd-data.js');
  const outMmd = resolve(outDir, 'erd.mmd');
  ...
  writeFileSync(outData, dataJs);
  writeFileSync(outMmd, mmd + '\n');
  ...
  return { data, mmd, entities: entityOut.length, relations: relations.length, modules: Object.keys(operations).length };
}

// CLI entry — `node scripts/generate-erd.mjs`
