...
| **Solo** | Python sidecar + Django ORM | `django.db.models` | Python (interpreted) |
| **Full** | Python sidecar + Django ORM | `django.db.models` | Python (interpreted) |
...
### After (Solo/Full — Sidecar Django ORM)
...
│  ┌────────────────────┐  ┌───────────────────┐  │
│  │ Hardware (printer, │  │ Sidecar lifecycle │  │
│  │  cash drawer,      │  │ (start/stop,      │  │
│  │  file I/O, dialogs)│  │  health check)    │  │
│  └────────────────────┘  └───────────────────┘  │
├─────────────────────────────────────────────────┤
│          Python Sidecar (primary data layer)     │
│  ┌──────────────────────────────────────────┐   │
...
## What Moves to the Sidecar vs What Stays in Rust

### Operations That Move to the Sidecar (Django ORM)

All Diesel-backed CRUD operations move to the Python sidecar. The React frontend communicates with the sidecar over HTTP REST (port 8765) instead of Tauri IPC for these operations:

> **Sync-tracking note:** Every Django model in the sidecar has `is_synced`, `sync_status`, and `synced_at` fields. When migrating data from Diesel, mark all imported records as `is_synced=True`, `sync_status="synced"` to prevent the sidecar from attempting to push historic records to the cloud. Fresh records created after migration will use the normal pending-sync flow.

| Rust Module | Operation | Sidecar Endpoint |
|-------------|-----------|-----------------|
...
| `sidecar` | Start/stop Python sidecar | Process lifecycle management |
...
| Diesel Table (Minimal) | Django Model (Solo/Full Sidecar) | Django Table | Match |
|------------------------|-----------------------------------|-------------|-------|
...
| `tax_reports` | — (not in sidecar) | — | ❌ No model yet |
| `report_metadata` | — (not in sidecar) | — | ❌ No model yet |
| `delivery_types` | — (not in sidecar) | — | ❌ No model yet |
| `employee_types` | — (not in sidecar) | — | ❌ No model yet |
| `loyalty_transactions` | — (handled by Customer model) | — | 🟡 Integrated |
| `inventory_alerts` | — (not in sidecar) | — | ❌ No model yet |
| `settings` | `DeviceConfig` | `full_device_configs` | ⚠️ Different approach |
| `recipe_ingredients` | — (merged into Recipe) | — | 🟡 Integrated |
| `recipe_types` | — (not in sidecar) | — | ❌ No model yet |

### Tables in Sidecar With No Diesel Equivalent (Solo/Full Only)

These tables exist only in the Sidecar's Django ORM. They have no corresponding Diesel table and do not need migration from Minimal:
...
The migration runs as a one-time Python script executed by the sidecar on first launch when it detects a legacy Diesel database.
...
The sidecar checks for a legacy Diesel database on startup:
...
The Diesel auth system uses a simple `users` table with bcrypt password hashes. The Django sidecar uses `django-allauth`. Migration:
...
All Django models in the sidecar include sync-tracking fields for cloud replication:
...
This is critical: without marking migrated records as synced, the sidecar's sync scheduler will attempt to push all historic data to the cloud on first run, potentially causing duplicates or stale-data conflicts.
...
The migration script lives at `sidecar/management/commands/migrate_from_diesel.py` (Django management command).
...
│ (Django ORM) │    │ + sidecar runs   │    │ (hot-reload)     │
└──────────────┘    └──────────────────┘    └──────────────────┘
                        │
┌──────────────┐    ┌───┴────────────────┐    ┌──────────────────┐
│ Edit Rust    │───▶│ cargo build (only  │───▶│ Tauri app with  │
│ (native ops) │    │ hardware module)   │    │ sidecar running │
└──────────────┘    └──────────────────┘    └──────────────────┘
```
...
|--------|------------------|--------------------|
| Build app | `cargo tauri build` | `make build-sidecar && cargo tauri build` |
| Run dev | `cargo tauri dev` | `make dev-desktop` (starts sidecar + Tauri) |
...
|-----------|-------------|---------|
| Python unit | Django model methods, validation | `uv run pytest sidecar/tests/` |
| Python integration | REST API endpoints | `uv run pytest sidecar/tests/ -k "api"` |
| Python migration | Diesel→Django data fidelity | `uv run pytest sidecar/tests/test_migration.py` |
| Rust unit | Hardware ops (printer, drawer) | `cargo test -- --nocapture` |
| Rust integration | Sidecar lifecycle commands | `cargo test --test integration` |
| E2E | Full app with sidecar running | `make test-e2e` |
...
1. Stop the sidecar: `kill <sidecar-pid>` or `invoke("stop_sidecar")`
...
- [ ] **Does the feature need database access?** → Implement in Python sidecar (Django ORM)
...
- [Sidecar Architecture](POS_ARCHITECTURE.md) — Sidecar process lifecycle details
