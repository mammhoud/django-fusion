# POS Server v2 — Robyn + Django ORM Architecture

> **Status:** Production Ready (with known test issues)  
> **Last Updated:** 2026-08-08 (pos-cloud server)  
> **Editions:** pos-solo (standalone) + pos-full (cloud master) — **both merged into `formint-pos/server/`**  
> **See also:** the Formint Cloud SaaS (`formint-cloud/`, package `formint-cloud`) serves the full API surface directly from Django on `:8767` — its Robyn server was removed in favour of the full Django setup — see [editions.md](architecture/editions.md#cloud--formint-cloud-formint-cloud)
...
Both POS editions now use a **Robyn** async Python web server with **Django ORM** for database access. This replaces the old Sanic-based server architecture.
...
Both servers ship a `manage.py` for Django migration management:
...
|---------|----------|----------|
| **Server file** | `server/server.py` | `server/server.py` |
...
|-----------|----------|-------|--------|
| `test_server.py` | `pos-full/server/tests/` | ~63 | 🟡 Collection errors |
| `test_unified_api.py` | `pos-solo/server/tests/` | ~155 | 🟡 Collection errors |
| `test_fragments.py` | `pos-full/server/tests/` | 8 | ✅ Added |
| `test_fusion.py` | `pos-full/server/tests/` | ~25 | ✅ Added |
| `test_fusion_integration.py` | `pos-full/server/tests/` | ~20 | 🟡 Collection errors |
| `test_webhook_e2e.py` | `pos-full/server/tests/` | — | 🔴 **3 collection errors** |
...
- [x] **Server fusion tests** — `test_fragments.py`, `test_fusion.py`, `test_fusion_integration.py`
