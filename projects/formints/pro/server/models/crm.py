"""
CRM models — Company, Contact, Pipeline, Stage, Deal, Activity, CRMNote (Full Edition).

Used by pos-full/server/routes/crm.py.
"""
...
class Stage(models.Model):
    ...
    pipeline = models.ForeignKey(
        Pipeline, on_delete=models.CASCADE, related_name="stages", verbose_name=_("pipeline"),
    )
    name = models.CharField(_("name"), max_length=200)
    display_order = models.IntegerField(_("display order"), default=0)
...
class Contact(models.Model):
    ...
    is_active = models.BooleanField(_("active"), default=True)
    pos_customer_id = models.IntegerField(_("POS customer ID"), blank=True, null=True, editable=False)
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
...
class Activity(models.Model):
    ...
    ACTIVITY_TYPES = [
        ("call", _("Call")), ("meeting", _("Meeting")), ("email", _("Email")),
        ("task", _("Task")), ("note", _("Note")), ("other", _("Other")),
    ]
    contact = models.ForeignKey(
        Contact, on_delete=models.SET_NULL, null=True, blank=True,
