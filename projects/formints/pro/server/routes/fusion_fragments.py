"""
POS Full — Fusion fragment rendering endpoints.
...
signals fragment-first mode (``fusion_render_first=true``).

Each endpoint delegates data retrieval to a ``FragmentComponent`` subclass
from the ``fragments/`` package, keeping the route handler focused on
transport (HTTP request/response) and the fragment class focused on
domain logic (model queries → context dict).

Usage (frontend)::

    <FusionProxy
      fragmentUrl={`${SERVER_BASE}/fusion/render/dashboard`}
...
# These are simple HTML snippets rendered with Django template syntax.
# They are kept inline (not in template files) so they work even when
# APP_DIRS is False or the template loader isn't configured for Django
...
def _render_template(template_source: str, context: dict[str, Any]) -> str:
    ...
    Uses the default Django template engine configured in ``settings.TEMPLATES``.
    """
