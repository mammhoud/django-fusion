...
- Name: admin-withdrawals.spec.ts >> Admin Withdrawal Approvals — E2E >> Reject Withdrawal >> cancels rejection when Cancel is clicked
- Location: tests/e2e/admin-withdrawals.spec.ts:367:9

# Error details

```
Error: locator.click: Error: strict mode violation: locator('button').filter({ hasText: 'Reject' }) resolved to 2 elements:
    1) <button class="px-3 py-1.5 text-sm rounded-lg transition-all font-medium capitalize bg-white border border-gray-200 text-gray-600 hover:bg-gray-50">rejected</button> aka getByRole('button', { name: 'rejected' })
    2) <button title="Reject withdrawal" class="inline-flex items-center gap-1 px-2.5 py-1.5 text-xs font-medium text-red-700 bg-red-50 hover:bg-red-100 rounded-lg transition-colors disabled:opacity-50">…</button> aka getByRole('button', { name: 'Reject', description: 'Reject withdrawal' })

Call log:
  - waiting for locator('button').filter({ hasText: 'Reject' })
        ...
        - main [ref=e46]:
          - generic [ref=e47]:
            ...
            - table [ref=e68]:
              ...
              - rowgroup [ref=e78]:
                - row "#40 Jane Doe $175 PayPal Jul 24, 2026 Pending Approve Reject" [ref=e79]:
                  - cell "#40" [ref=e80]
