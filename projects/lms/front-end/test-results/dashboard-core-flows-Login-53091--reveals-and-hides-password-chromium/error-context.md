...
- Name: dashboard-core-flows.spec.ts >> Login Page – Interactions >> password toggle reveals and hides password
- Location: tests/e2e/dashboard-core-flows.spec.ts:98:7
...
Error: strict mode violation: locator('input[type="text"]') resolved to 2 elements:
    1) <input value="" type="text" required="" autofocus="" id="username" class="input-field" placeholder="Enter your username"/> aka getByRole('textbox', { name: 'Username' })
    2) <input value="" type="text" required="" id="password" class="input-field pr-10" placeholder="Enter your password"/> aka getByRole('textbox', { name: 'Password' })
...
```ts
  9   |  *   - /registration (role selection, password match validation)
  10  |  *   - /student-dashboard (stats, quick nav, enrolled courses list)
  11  |  *   - /student-dashboard/enrolled-courses (filter buttons, search)
  12  |  *   - /student-dashboard/history (timeline, filter dropdown)
  13  |  *   - /student-dashboard/reviews (star rating, comment, submit)
  14  |  *   - /student-dashboard/wishlist (course cards, remove, empty state)
  15  |  */
  16  | 
  17  | import { test, expect } from "@playwright/test";
  18  | 
  19  | // ── Helper: collect console errors ──────────────────────────────────
  20  | async function captureConsoleErrors(page: import("@playwright/test").Page) {
  ...
  26  | }
  27  | 
  28  | function filterNextJSWarnings(errors: string[]) {
  29  |   return errors.filter(
  30  |     (e) =>
  31  |       !e.includes(" hydration ") &&
  32  |       !e.includes("Warning:") &&
  33  |       !e.includes("next") &&
  34  |       !e.includes("favicon") &&
  35  |       !e.includes("404")
  36  |   );
  37  | }
  38  | 
