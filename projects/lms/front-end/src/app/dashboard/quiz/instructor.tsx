        ...
        <div className="flex gap-2 mb-6">
          {['all', 'Active', 'Draft', 'Archived'].map((f) => (
            <button key={f} onClick={() => setFilter(f as any)}
              className={`px-4 py-2 text-sm rounded-lg transition-all font-medium ${filter === f ? 'bg-[rgb(var(--ctc-primary))] text-white' : 'bg-white text-gray-600 hover:bg-gray-50 border border-gray-200'}`}>
              {f === 'all' ? 'All' : f}
            </button>
          ))}
        </div>
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-1">
                    ...
                    <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${quiz.status === 'Active' ? 'bg-green-100 text-green-700' : quiz.status === 'Draft' ? 'bg-yellow-100 text-yellow-700' : 'bg-gray-100 text-gray-600'}`}>{quiz.status}</span>
                  </div>
                  ...
                  <div className="flex items-center gap-6 mt-3 text-sm text-gray-600">
                    ...
                    <span className="flex items-center gap-1">Avg: <span className="font-medium">{quiz.avgScore}%</span></span>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  <button className="p-2 text-gray-400 hover:text-[rgb(var(--ctc-primary))] hover:bg-[rgb(var(--ctc-primary))]/5 rounded-lg transition-colors"><HiPencil className="w-4 h-4" /></button>
                  <button className="p-2 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors"><HiTrash className="w-4 h-4" /></button>
                  <button className="p-2 text-gray-400 hover:text-gray-600 hover:bg-gray-50 rounded-lg transition-colors"><HiDotsVertical className="w-4 h-4" /></button>
                </div>
              </div>
            </div>
          ))}
