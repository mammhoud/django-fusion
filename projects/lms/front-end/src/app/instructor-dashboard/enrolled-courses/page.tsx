            ...
            {enrollments.map((e, idx) => (
              <motion.tr key={e.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: idx * 0.05 }} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-4">
                  <div className="flex items-center gap-3"><div className="w-full max-w-xs bg-gray-200 rounded-full h-2"><div className="progress-fill" style={{ width: `${e.progress}%` }} /></div><span className="text-sm font-medium text-gray-700">{e.progress}%</span></div>
                </td>
              </motion.tr>
            ))}
          </tbody>
