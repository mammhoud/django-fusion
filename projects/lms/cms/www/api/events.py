...
def _serialize_event(event) -> dict:
    ...
    return {
        ...
        "start_date": event.start_date.isoformat() if hasattr(event, "start_date") and event.start_date else "",
        "end_date": event.end_date.isoformat() if hasattr(event, "end_date") and event.end_date else "",
        ...
        "capacity": getattr(event, "capacity", 0) or getattr(event, "max_participants", 0) or 0,
        "registered_count": getattr(event, "registered_count", 0) or 0,
        "status": getattr(event, "status", "upcoming") or "upcoming",
        "category": getattr(event, "category", "") or getattr(event, "event_type", "") or "",
...
def event_list(request):
    ...
    items, pagination = paginate_queryset(events, request) if hasattr(events, "count") else ([], {"page": 1, "per_page": 20, "total": 0, "total_pages": 0})
    return {
        "status": "success",
