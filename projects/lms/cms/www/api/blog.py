...
def _serialize_post(post: BlogPost) -> dict:
    ...
    return {
        ...
        "category_name": post.categories.first().name if post.categories.exists() else "Uncategorized",
...
def blog_post_list(request):
    ...
    return {
        ...
        "next": None,
        "previous": None,
        "results": [_serialize_post(p) for p in items],
    }
...
def blog_post_detail(request, pk):
    ...
    data["related_posts"] = [_serialize_post(p) for p in post.get_related_posts(limit=3)]
