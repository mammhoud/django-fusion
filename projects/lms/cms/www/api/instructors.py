...
def _serialize_instructor(user: User) -> dict:
    ...
    return {
        ...
        "avatar": "",
        "title": (
            "Instructor" if user.groups.filter(name="Instructors").exists() else ""
        ),
...
def instructor_list(request):
    ...
    return paginated_response(
        items, pagination, request, [_serialize_instructor(u) for u in items]
    )
...
def instructor_dashboard(request, pk):
    ...
    avg_rating = (
        Review.objects.filter(course__in=published_courses).aggregate(
            avg=Avg("rating")
        )["avg"]
        or 0
    )
