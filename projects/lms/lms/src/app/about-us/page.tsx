...
function AboutPageContent({ page }: { page: CmsPage | undefined }) {
  ...
  return (
    <div>
      {/* Hero */}
      <section className="bg-gradient-to-br from-[rgb(var(--ctc-primary))] to-[rgb(var(--ctc-primary-dark))] text-white py-20">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {highlights.map((h, i) => (
            <motion.div key={h.label} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }}
              ...
              <h.icon className="w-8 h-8 text-[rgb(var(--ctc-primary))] mx-auto mb-3" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {(mission?.items || []).map((item, i) => (
            <motion.div key={item.heading} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }}
              ...
              <div className="w-12 h-12 bg-[rgb(var(--ctc-primary))]/10 rounded-full flex items-center justify-center mx-auto mb-4">
                <HiStar className="w-6 h-6 text-[rgb(var(--ctc-primary))]" />
              </div>
