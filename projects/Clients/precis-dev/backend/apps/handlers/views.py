...
class LandingPageView(PageHandler):
        ...
        context.update(
            {
                ...
                "localized_blocks": self._get_localized_blocks(page, language),
                "localized_body": self._get_localized_body(page, language),
    ...
    def _get_localized_title(self, page, language: str) -> str:
        """Return the translated title used by HTML headings and breadcrumbs."""
        translation = self._get_translation(page, language)
        if translation and translation.title:
            return translation.title
        return page.title

    def _get_translation(self, page, language: str):
        """Resolve the requested translation, falling back explicitly to English."""
    ...
    def _get_localized_body(self, page, language: str) -> str:
        """Return translated body text without trusting editor-supplied HTML."""
        translation = self._get_translation(page, language)
        if translation and translation.body:
            return strip_tags(translation.body)
        return ""
    ...
    def _get_breadcrumbs(self, page) -> list[dict]:
        ...
        def label_for(target, fallback):
            return self._get_localized_title(target, language) if target else fallback

        page_name = page.__class__.__name__
        if page_name in ("TeamPage", "FounderPage", "StartupPage"):
            about = page.get_parent().specific
            return [
                {"label": label_for(about.get_parent().specific, "Home"), "href": "/"},
                {"label": label_for(about, "About"), "href": "/about/"},
            ]
        if page_name == "PhasePage":
            services = page.get_parent().specific
            home = services.get_parent().specific
            return [
                {"label": label_for(home, "Home"), "href": "/"},
                {"label": label_for(services, "Services"), "href": "/services/"},
            ]
        if page_name == "PromptPage":
            phase = page.get_parent().specific
            services = phase.get_parent().specific
            home = services.get_parent().specific
            return [
                {"label": label_for(home, "Home"), "href": "/"},
                {"label": label_for(services, "Services"), "href": "/services/"},
                {"label": label_for(phase, phase.title), "href": "/about/services/"},
            ]
        if page_name == "ProductPage":
            products = page.get_parent().specific
            home = products.get_parent().specific
            return [
                {"label": label_for(home, "Home"), "href": "/"},
                {"label": label_for(products, "Products"), "href": "/products/"},
            ]
        if page_name == "BlogPostPage":
            blog = page.get_parent().specific
            home = blog.get_parent().specific
            return [
                {"label": label_for(home, "Home"), "href": "/"},
                {"label": label_for(blog, "Blog"), "href": "/blog/"},
            ]
        return []
    ...
    def _get_nav_items(self) -> list[dict]:
        ...
        def translated_label(href, fallback):
            slug = str(href).strip("/").split("/")[-1] or "home"
            target = landing_pages.filter(slug=slug).first()
            translation = PageTranslation.for_page(target, language) if target else None
        ...
        for item in landing_module.get_navigation_context(self.request):
            ...
            clean["label"] = translated_label(clean.get("href", ""), clean.get("label", ""))
            clean["children"] = clean.get("children") or []
            for child in clean["children"]:
                child["label"] = translated_label(child.get("href", ""), child.get("label", ""))
            items.append(clean)
        return items
...
class AboutServicesView(LandingPageView):
    """About → Services subpage (/about/services/) — the consolidated delivery
    phase document. Reuses the services template so the offering + phases read
    as one page, and marks the About-section subnav as active."""

    template_name = "pages/services.html"

    def _get_page(self) -> ServicesPage:
        return ServicesPage.objects.first() or AboutPage.objects.first() or HomePage.objects.first()

    def get_context_data(self, request: HttpRequest | None = None, **kwargs) -> dict:
        context = super().get_context_data(request=request, **kwargs)
        context["about_subnav_current"] = "services"
        context["breadcrumbs"] = [
            {"label": "Home", "href": "/"},
    ...
    def get_context_data(self, request: HttpRequest | None = None, **kwargs) -> dict:
        ...
        context["breadcrumbs"] = [
            *context.get("breadcrumbs", []),
            {
                "label": context.get("localized_content", {}).get("title", page.title),
                "href": f"/products/{page.slug}/",
            },
        ]
...
class PhasePageView(LandingPageView):
    ...
    def get_context_data(self, request: HttpRequest | None = None, **kwargs) -> dict:
        context = super().get_context_data(request=request, **kwargs)
        phase = context["page"]
        localized = context.get("localized_content", {})
        outcomes = localized.get("outcomes")
        if not isinstance(outcomes, list):
            outcomes = [line.strip() for line in phase.outcomes.splitlines() if line.strip()]
        context["phase_outcomes"] = outcomes
        language = context.get("content_language", "en")
        phase_prompts = []
        for prompt in PromptPage.objects.live().child_of(phase).order_by("title"):
            prompt_data = _apply_page_translation(prompt, _page_to_dict(prompt), language)
            phase_prompts.append(
                {
                    "title": prompt_data.get("title", prompt.title),
                    "slug": prompt.slug,
                    "href": "/about/services/",
                }
            )
        context["phase_prompts"] = phase_prompts
        return context
...
class PromptPageView(LandingPageView):
    ...
    def get_context_data(self, request: HttpRequest | None = None, **kwargs) -> dict:
        context = super().get_context_data(request=request, **kwargs)
        prompt = context["page"]
        phase = prompt.get_parent().specific
        language = context.get("content_language", "en")
        context["phase"] = phase
        context["localized_phase"] = _apply_page_translation(
