...
class Organization(DefaultBase, ClusterableModel):
    ...
    def get_services(self):
        ...
        # Lazy load Service model to avoid circular imports
        try:
            Service = apps.get_model('shared', 'Service')
            return Service.objects.filter(
                Q(owner_department__company=self) |
                Q(supporting_departments__company=self)
            ).distinct()
        except LookupError:
            # Service model not available in this project
            return []
...
class Department(DefaultBase, ClusterableModel):
    ...
    def get_employee_count(self):
        ...
        # Lazy load TeamMembership model to avoid circular imports
        try:
            ...
            # TeamMembership model not available in this project
            return 0
