# Precis Dev — AI Agent Instructions

**Path:** `projects/precis/precis-dev/`  
...
project-level source of truth for the development copy of the unified Precis product.
...
│   ├── src/pages/            # Marketing/catalog/LMS routes
...
│   ├── apps/components/      # Templatetag helpers
│   ├── apps/domain/          # Shared domain model layer
│   ├── apps/auth/            # Allauth adapters and auth integration
│   ├── apps/tasks/           # Background task execution
...
├── configs/                  # YAML config cascade (site, admin, defaults)
...
**This is a development copy (precis-dev).** It mirrors the unified
precis-main product for safe experimentation. Changes validated here can be
promoted to precis-main once approved.

The primary domain for this instance is `dev.structa.cloud`. Use
`WEBSITE=precis-dev` in the projects dispatcher:

```bash
cd projects && make check WEBSITE=precis-dev
```
...
Use the root projects dispatcher for cross-product commands:

```bash
cd projects && make run-dev WEBSITE=precis-dev
cd projects && make check WEBSITE=precis-dev
cd projects && make migrate WEBSITE=precis-dev
```
...
- `docs/projects/precis/precis-landing/` contains deployment and API notes.
