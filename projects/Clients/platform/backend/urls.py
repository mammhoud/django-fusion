...
urlpatterns = [
    ...
    # ── LMS catalog API — merged LMS frontend course pages (/api/courses/*)
    # Served from the same Course model with the paginated LMS contract
    # (fetchCourseList / fetchCourseDetail in the Astro frontend).
    path("api/courses/", courses_api.list_courses, name="api_courses_list"),
    path("api/courses/filters/", courses_api.course_filters, name="api_courses_filters"),
    path("api/courses/<slug:slug>/", courses_api.course_detail, name="api_courses_detail"),
    ...
    path(
        "tasks/",
        TaskCenterView.as_view(site_name="precis-dev", template_name="tasks/task_center.html"),
        name="tasks",
    ),
