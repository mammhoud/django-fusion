# Precis Landing — Template Architecture
...
Precis Landing uses a **dual-rendering** architecture: the same page can be served as
...
Django-side JS/SCSS is bundled by webpack via `webpack/precis-landing.config.js`:
...
| `webpack/precis-landing.config.js` | Webpack config for Django-side SCSS/JS |
