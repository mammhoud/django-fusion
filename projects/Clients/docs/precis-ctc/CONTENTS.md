...
| `/products/` | Medical learning products and catalog entry points | Localized Wagtail `MarketingPage` data exposed through the landing API; empty sections stay empty rather than using frontend fallback copy |
| `/products/[slug]/` | Product/course detail, outcomes, audience, enrollment CTA, media/reference gallery | Product API and learning models; the former “Models & snippets” code panel is now a medical media gallery |
