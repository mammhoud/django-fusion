...
### 5.2 Precis Landing — Uses `{% include %}` Instead

Precis Landing does NOT use `{% comp %}`. Its `base.html` uses traditional `{% include %}`:
...
Precis uses this pattern (from `configs/base/templates.py`). Precis Landing registers
...
| `projects/structa.cloud/assets/templates/` | Primary comp user — all layout skeletons + components |
