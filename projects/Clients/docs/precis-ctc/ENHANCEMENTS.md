...
### SEO metadata & Wagtail-driven content

- `SiteSettings` now carries `og_type`, `robots_meta`, and `canonical_url`
  alongside the existing SEO fields; `get_seo_context()` exposes robots +
  canonical + og type, and `/apis/site/settings/` returns them.
- Every page API response (`/apis/pages/<slug>/`) now includes a `seo` object
...
- Both render roads emit the full meta set from backend data:
  `base.html` (Django) renders description/keywords/author/robots/OG/Twitter/
  canonical via `seo_context_processor` + the Wagtail serve hook; the Astro
  `Layout.astro` resolves the same fields from `pageData.seo` +
  `siteSettings`. No hardcoded meta copy remains.
- About page content is fully Wagtail-driven: `AboutPage.facts` accepts
  `mission`, `skills`, and `faq` blocks, the fixture seeds all three across
  all 7 locales, and the API exposes `mission_values`/`skills`/`faq`.
- Swedish (SV) home + About translations seeded; the About fixture now covers
  EN/AR/DE/ES/FR/PT-BR/SV.
- Hardcoded fallback arrays removed from the About, FAQ, Pricing, Features,
  Projects and Products Astro pages — sections render backend data only.

### Admin & proxy routing

- Traefik backend router now matches exact `Path(/admin)` and
  `Path(/django-admin)` so Django’s `APPEND_SLASH` redirects instead of the
  Astro catch-all serving a 404.
- Verified live: `/admin` → Wagtail admin; `/django-admin` → Django admin
  (Unfold).
...
7. ~~Add locale-aware metadata, canonical URLs, `hreflang`, OpenGraph images, and structured data for articles/events/courses.~~
   - **Done (core):** per-page `seo` block, site-level robots/canonical/OG
     defaults, and backend-driven meta on both render roads. Remaining
     deltas: `hreflang` alternates, JSON-LD structured data, and per-article/
     per-event OG images.
