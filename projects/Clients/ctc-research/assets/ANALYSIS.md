...
|-----------|---------|-----|
| `static/` | **All projects** (lms, cms-fusion, precis-lms, portfolio, cypercloud, fusion-cms) | Via `STATICFILES_DIRS` → `SHARED_STATIC_DIR` in `assets.py`. Served by Nginx `shared-proxy` at `/static/`. |
| `templates/` | **All projects** (lms, cms-fusion, precis-lms, portfolio, cypercloud, fusion-cms) | Via `TEMPLATES_DIRS` → `BASE_DIR.parent / "assets" / "templates"` in `templates.py`. Served by Django template engine. |
...
|--------|-------------|-------|
| `projects/precis-lms/assets copy/` | ~1,198 files | Core shared assets (static, templates, locale, fixtures, scripts) |
...
| `projects/precis-lms/backend/assets/` | 71 unique files | Same content as cms-fusion (derived from same source) |
...
| `projects/precis-lms/assets/` | Isolated asset copy for Fusion LMS project |
