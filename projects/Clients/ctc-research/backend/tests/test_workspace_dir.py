...
The historical ``parents[6]`` heuristic resolved the monorepo root on the host
(``<repo>/projects/precis/precis-ctc/backend/configs/settings/conf.py`` is seven levels
deep), but overran with ``IndexError`` inside the container
(``/app/precis-lms/configs/settings/conf.py`` is only four levels deep). That
...
class TestWorkspaceDirHostLayout:
    """Repository checkout on the build machine (seven levels deep)."""
    ...
    def test_resolves_to_dir_owning_libs(self, monkeypatch, tmp_path):
        ...
        (tmp_path / "libs").mkdir()
        fake = _fake_conf(
            monkeypatch, tmp_path, "projects/precis/precis-lms/backend/configs/settings/conf.py"
        )
        ...
        # Sanity: the fake file is genuinely seven levels deep, as on the host.
        assert len(fake.resolve().parents) > 5

    def test_matches_historical_parents6(self, monkeypatch, tmp_path):
        """On the host layout the new walk-up agrees with the old parents[6]."""
        (tmp_path / "libs").mkdir()
        fake = _fake_conf(
            monkeypatch, tmp_path, "projects/precis/precis-lms/backend/configs/settings/conf.py"
        )

        assert conf._resolve_workspace_dir() == fake.resolve().parents[6]


class TestWorkspaceDirContainerLayout:
    """Baked image layout: /app/precis-lms/configs/settings/conf.py (4 deep)."""
    ...
    def test_resolves_to_app_when_libs_present(self, monkeypatch, tmp_path):
        ...
        fake = _fake_conf(
            monkeypatch, tmp_path, "app/precis-lms/configs/settings/conf.py"
        )
    ...
    def test_old_parents5_would_crash_here(self, monkeypatch, tmp_path):
        ...
        container_conf = Path("/app/precis-lms/configs/settings/conf.py")
        ...
        (tmp_path / "app" / "libs").mkdir(parents=True)
        _fake_conf(monkeypatch, tmp_path, "app/precis-lms/configs/settings/conf.py")
