...
- **Cleaned empty directories**: `core/`, `appspecific/`, `ui/wagtailcore/`, `events/sections/`
- Preserved `allauth.md` (comprehensive allauth template mapping reference)

### Settings Inlining — Per-Site Architecture

- **Inlined `LMS_LOCAL_APPS`** from `configs/base/lms.py` into `ctc-research/settings.py` and `lms-demo/settings.py`
