#!/bin/bash
# =============================================================================
# Domain Drift Check — Ensures cms-fusion and lms-fusion domain code stays in sync
#
# The `apps/core/domain/` directory is intentionally duplicated between
# cms-fusion and lms-fusion. This script verifies they are identical
...
LMS_DOMAIN="$WORKSPACE/projects/lms-fusion/backend/apps/core/domain"
...
if [ ! -d "$LMS_DOMAIN" ]; then
    echo "❌ ERROR: lms-fusion domain directory not found: $LMS_DOMAIN"
    exit 2
fi
...
while IFS= read -r file; do
    ...
    if [ ! -f "$lms_file" ]; then
        DRIFT_FOUND=true
        ISSUES+=("MISSING in lms-fusion: $file")
    fi
...
if [ "$DRIFT_FOUND" = true ]; then
    ...
    echo "║  cms-fusion and lms-fusion domain code has diverged!             ║"
    echo "╚══════════════════════════════════════════════════════════════════╝"
    echo ""
...
# ── Success ──────────────────────────────────────────────────────────────────
if [ "$QUIET" = false ]; then
    echo "  ✅ Domain drift check passed — cms-fusion and lms-fusion are in sync"
fi
exit 0
