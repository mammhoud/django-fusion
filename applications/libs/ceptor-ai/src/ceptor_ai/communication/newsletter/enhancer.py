...
1. ``ceptor_ai.ai`` (built-in, Django-integrated) — default.
2. ``ceptorai.ai`` (optional, standalone) — used when
   ``RSEAL_AI_BACKEND = "ceptorai"`` is set in Django settings, or when
   ``backend="ceptorai"`` is passed to the constructor.

This makes ``ceptorai`` an **optional** dependency: if it is not installed
...
Usage::
    ...
    from ceptor_ai.communication.newsletter.enhancer import NewsletterEnhancer

    # Default — uses ceptor_ai.ai (OpenAI or Claude via Django settings)
    ...
    # Optional ceptorai backend
    enhancer = NewsletterEnhancer(backend="ceptorai", api_key="sk-...")
    subject  = enhancer.enhance_subject("Monthly product update")
"""
...
def _get_ai_registry(backend: str):
    ...
    Args:
        backend: ``"rseal"`` (default) or ``"ceptorai"``.
    ...
    if backend == "ceptorai":
        try:
            ...
            logger.warning(
                "ceptorai is not installed. "
                "AI features will be unavailable. "
                "Install it with: pip install ceptorai[openai]"
            )
            return None
    ...
    try:
        ...
        try:
            from ceptor_ai.ai.integrations import AIIntegrationRegistry  # noqa: PLC0415
            return AIIntegrationRegistry
        except ImportError:
            logger.warning(
                "No AI backend available. "
                "Install ceptorai with: pip install ceptorai[openai]"
            )
            return None
...
class NewsletterEnhancer:
    ...
    2. ``RSEAL_AI_BACKEND`` Django setting (``"rseal"`` or ``"ceptorai"``).
    ...
    Args:
        ...
        backend: ``"rseal"`` or ``"ceptorai"``.  Overrides the Django setting.
