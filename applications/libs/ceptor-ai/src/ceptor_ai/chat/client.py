"""
ceptor_ai.chat.client
======================

REST API client and chat bubble interface for the ceptor-ai server.

Classes
-------
CeptorClient
    Low-level HTTP client for the ceptor-ai REST API.
ChatBubble
...
class CeptorClient:
    """
    Low-level REST client for the ceptor-ai server.

    Uses only the Python standard library (urllib) — no requests dependency.

    Args:
        base_url: Base URL of the ceptor-ai server (e.g. ``"http://localhost:8765"``).
    ...
    def _request(
        ...
        try:
            ...
            logger.error("CeptorClient HTTP %s %s → %d: %s", method, url, exc.code, raw)
            raise
        except urllib.error.URLError as exc:
            logger.error("CeptorClient connection error %s %s: %s", method, url, exc.reason)
            raise
...
class ChatBubble:
    """
    High-level chat interface for the ceptor-ai server.

    Maintains a session and provides a simple ``send()`` / ``history`` API.

    Args:
        server_url: Base URL of the ceptor-ai server.
    ...
    def __init__(
        ...
        import uuid
        self.client = CeptorClient(base_url=server_url, timeout=timeout)
    ...
    def send(self, message: str, **kwargs) -> ChatMessage:
        """
        Send a message to the ceptor-ai server and return the reply.
    ...
    def is_connected(self) -> bool:
        """Return True if the ceptor-ai server is reachable."""
