# ceptor-ai Agent and Customizer Guide

## Merged package structure

`ceptor-ai` now ships all runtime code from `src/ceptor_ai/`. Former `ceptor-ai` modules are available under the merged `ceptor_ai` namespace, and new AI-facing customizer and MCP tools live in `ceptor_ai.customizer` and `ceptor_ai.mcp`.
...
`ceptor_ai.mcp_server` is the source of truth for framework-agnostic MCP
