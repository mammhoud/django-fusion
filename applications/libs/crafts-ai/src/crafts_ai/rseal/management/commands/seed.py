...
class Command(AppCommand):
    ...
    def handle_app_config(self, app_config, **options):
        ...
        # Import Seeder lazily — requires django-grep to be installed (dev/test only)
        # Note: this import is intentionally lazy and only used in dev/test environments
        try:
            import importlib
            django_grep_seeder = importlib.import_module('django_grep.seeder')
            Seeder = django_grep_seeder.Seeder
        except ImportError:
            raise SeederCommandError(
                'django-grep is required for seeding. Install it as a dev dependency: '
