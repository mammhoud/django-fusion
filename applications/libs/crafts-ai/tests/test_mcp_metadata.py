from crafts_ai.mcp_server import MCP_FEATURES, MCP_FILE_STRUCTURE, latest_features
...
def test_latest_features_exposes_documentation_file_structure() -> None:
    ...
    assert metadata["package"] == "crafts-ai"
    assert "docs/ai/latest_features.md" in metadata["file_structure"]["docs"]
    assert "applications/libs/crafts-ai/src/crafts_ai/mcp_server.py" in (
