...
def test_classifies_ai_import_for_crafts_ai() -> None:
    rule = classify_import("crafts_ai.rseal.ai.integrations")
...
def test_keeps_wagtail_blocks_in_crafts_ai_rseal() -> None:
    rule = classify_import("crafts_ai.rseal.blocks.contact.contact_card")
    assert rule is not None
    assert rule.target == "crafts_ai.rseal.blocks"
...
def test_migration_plan_finds_imports(tmp_path: Path) -> None:
    ...
    module.write_text(
        "from crafts_ai.rseal.ai.integrations import OpenAIIntegration\n"
