"""Tests for ``django_fusion.analyzer.views``.
...
class TestCoerceDepth:
    ...
    def _import(self):
        from django_fusion.analyzer.views import MAX_DEPTH, _coerce_depth
...
class TestCoerceBool:
    @pytest.fixture(autouse=True)
    def _import(self):
        from django_fusion.analyzer.views import _coerce_bool
...
class TestCoerceFilters:
    @pytest.fixture(autouse=True)
    def _import(self):
        from django_fusion.analyzer.views import _coerce_filters
...
class TestAnalyzeRequestDataclass:
    def test_from_payload_no_longer_exists(self):
        ...
        from django_fusion.analyzer.schemas import AnalyzeRequest
...
class TestAnalyzeViewPost:
    ...
    *requires* ``tmp_path`` (for response-code tests that don't write
    ...
    def test_empty_body_returns_400(self, tmp_path):
        ...
        from django_fusion.analyzer.views import AnalyzeView
