...
from django_fusion.site.context.settings import SETTINGS
...
def is_htmx_request(request: HttpRequest) -> bool:
    ...
    This is the single canonical HTMX check used throughout django-fusion.
    All other HTMX detection (``resolve_strategy``, ``FragmentComponent``,
    ``FragmentDetector``) delegates here.
    """
    ...
    * ``"document"`` → :meth:`_render_layout_response`
    ...
    def _render_layout_response(self, context: dict[str, Any]) -> HttpResponse:
        ...
        try:
            ...
            return JsonResponse({"error": "Rendering failed", "details": str(exc)}, status=500)
...
class WagtailPageMixin(FragmentHandlerMixin):
    ...
    by the osoul pipeline.  ``get_context()`` is still the Wagtail-style
    ...
    def serve(self, request: HttpRequest, *args: Any, **kwargs: Any) -> HttpResponse:
        ...
        try:
            ...
            if self.strategy == "fragment" and getattr(request, "is_unpoly", False):
                try:
                    request.up.set_title(  # type: ignore[attr-defined]
    ...
    def _wagtail_render(
        ...
        """
        if self.strategy == "fragment":
            return self._render_fragment_response(request, context)
    ...
    def get_context(self, request: HttpRequest, *args: Any, **kwargs: Any) -> dict[str, Any]:  # type: ignore[override]
        """
        Extend Wagtail's get_context() with osoul pipeline keys.
