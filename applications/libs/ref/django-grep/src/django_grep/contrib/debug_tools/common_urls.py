"""
Common URL patterns shared across all Django/Wagtail projects.

``configure_common_urls(urlpatterns)`` adds:
  - Language switching  (set-language/, i18n/)
  - Wagtail sitemaps   (sitemap.xml, sitemap-<section>.xml)
  - Django sitemaps    (site.xml, site-<section>.xml)
  - robots.txt
...
def configure_common_urls(urlpatterns: list, sitemaps: dict | None = None) -> list:
    ...
    if _wagtail_sitemaps:
        updated += [
            path("sitemap.xml", wagtail_sitemap_index, {"sitemaps": {}}, name="wagtail-sitemap-index"),
            path("sitemap-<section>.xml", wagtail_sitemap, {"sitemaps": {}}, name="wagtail-sitemap"),
        ]
