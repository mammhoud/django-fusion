# Merge Conflicts — django-rseal

This file documents symbol/file conflicts encountered during the migration of `django-seed` and `django-grep` into `django-rseal`. Each entry requires a developer decision before the conflict is considered resolved.

---

## Conflict: templates/emails/invitation.html

- Source A: `libs/django-rseal/src/django_rseal/templates/emails/invitation.html` — Minimal HTML template using `registration_url` context variable; inline styles only; no role/group display.
