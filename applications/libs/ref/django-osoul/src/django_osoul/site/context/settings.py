"""
django_osoul.contrib.context.settings
======================================
...
Projects that use ``django_rseal`` should override this context processor
...
Add to ``TEMPLATES[0]["OPTIONS"]["context_processors"]`` in ``settings.py``::
    ...
    "django_osoul.contrib.context.settings.SETTINGS",

Or override in your project's context processor::

    from django_osoul.contrib.context.settings import SETTINGS as base_settings
...
def SETTINGS(request=None) -> dict:  # noqa: N802  (uppercase matches Django convention)
    ...
    ``django_rseal`` should override this in their own context processor
