...
Drift-prone vs hooking into `django_osoul.comp.templatetags.tags.block` because
...
def parse_template(content: str) -> ParsedTemplate:
    ...
    # then dedup by (path, opener_position) and sort by opener_position so
    ...
    return ParsedTemplate(extends=extends, blocks=blocks, comps=comps)
