"""
Fragment Components for django-osoul
=====================================

``FragmentComponent`` extends ``RoutableComponent`` for HTMX-only partial
...
* HTMX detection delegates to the single canonical helper
  ``django_osoul.site._context_mixins.is_htmx_request``.
...
class FragmentComponent(RoutableComponent):
    """
    HTMX-only fragment component registered in the ``/osoul/`` routing tree.
    ...
    * ``htmx_only`` — return HTTP 400 for non-HTMX requests when ``True``.
    ...
    * ``dispatch()`` — proper method override (no monkey-patching).
    ...
    def dispatch(self, request: HttpRequest, *args: Any, **kwargs: Any) -> HttpResponse:
        """Gate non-HTMX requests when ``htmx_only=True``.
        ...
        """
        if self.htmx_only and not is_htmx_request(request):
            return HttpResponse(
                "This endpoint only accepts HTMX requests.", status=400
            )
    ...
    def setup(self, request: HttpRequest, *args: Any, **kwargs: Any) -> None:
        """Force strategy to ``"fragment"`` for HTMX requests."""
    ...
    def get(self, request: HttpRequest, *args: Any, **kwargs: Any) -> HttpResponse:
        ...
        * HTMX request → ``render_fragment_response()`` (fragment + OOB)
        * Non-HTMX request → ``render_response()`` → ``_render_layout_response()``
        """
        if is_htmx_request(request):
    ...
    def render_oob_fragment(
        ...
        """
        template_path = fragment_name.replace(".", "/") + ".html"
        ctx = {**context, "fragment_id": element_id}
        html = render_to_string(template_path, ctx, request=self.request)
    ...
    def get_template_names(self) -> list[str]:
        """Return the fragment template for HTMX, full-page template otherwise.

        Uses ``fragment_name`` (dotted) → path conversion.  Falls back to
        ``template_name`` for non-HTMX requests.
        """
        if is_htmx_request(self.request) and self.fragment_name:
