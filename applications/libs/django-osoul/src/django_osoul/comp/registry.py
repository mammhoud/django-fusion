...
``django_osoul``'s ``{% comp "name" %}`` world for cases where the
...
- :func:`register_default_partials` — default partials-path discovery
  wired into ``django_osoul.comp.apps.CoreExtAppConfig.ready()``; sites
...
from django_osoul.comp.core._init import Component, components

logger = logging.getLogger(__name__)


_DEFAULT_INCLUDE_PATH_ROOTS: Final[tuple[str, ...]] = ("partials",)
...
def _iter_default_include_path_roots() -> Iterator[Path]:
    ...
    ``django_osoul.comp.apps`` AppConfig is registered.
    """
