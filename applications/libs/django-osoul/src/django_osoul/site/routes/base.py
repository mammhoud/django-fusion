...
"""
Base routing classes and utilities for django-osoul.

This module provides the core routing infrastructure including:
- BaseViewset: Base class for all viewsets
- Viewset: Declarative class-based routing with metaclass
- Route: URL route with prefix and viewset
- menu_path: Create URL patterns with menu metadata
- IndexViewMixin: Automatic index URL redirection

Classes:
    BaseViewset: Base class for defining class-based Django routing.
    Viewset: Declarative class-based routing configuration.
    ViewsetMeta: Metaclass that collects URL patterns from classes.
    Route: Represents a URL route with prefix and viewset.
    _UrlName: String wrapper for URL name metadata.
    _URLResolver: Custom URL resolver with extra metadata.
    _IndexRedirectView: Redirects to first suitable URL pattern.

Functions:
    route: Create a route mapping a URL prefix to a viewset.
    menu_path: Create a URL pattern with menu metadata.
    detect_fragment_strategy: Detect HTMX rendering strategy.
    add_fragment_detection_to_request: Annotate request with HTMX detection.
"""
...
from django_osoul.contrib import (
    DEFAULT,
    camel_case_to_underscore,
    list_path_components,
    strip_suffixes,
)
from django_osoul.site.utils import viewprop
...
class BaseViewset:
    ...
    Notes:
        ``_parent`` is stored in ``__dict__`` directly (not via a slot) so that it
        survives even when Django's ``View.__init__`` breaks the cooperative MRO
        chain by not calling ``super().__init__()``.  ``RoutableComponent`` and any
        other class that mixes ``BaseViewset`` with a Django ``View`` subclass must
        call ``BaseViewset._init_viewset(self)`` from their own ``__init__`` to
        guarantee the attribute is present.
    """
    ...
    def _init_viewset(self) -> None:
        """Ensure viewset bookkeeping attrs are present on this instance.

        Safe to call multiple times — subsequent calls are no-ops.
        This exists so that ``RoutableComponent`` (which inherits from both
        ``View`` and ``BaseViewset``) can guarantee initialisation even when
        ``View.__init__`` does not propagate ``super().__init__()``.
        """
    ...
    def parents(self) -> list[BaseViewset]:
        ...
        """
        result = []
        current = self.__dict__.get("_parent")
        while current is not None:
            result.append(current)
            current = current.__dict__.get("_parent")
...
class Viewset(BaseViewset, metaclass=ViewsetMeta):
    ...
    Attributes:
        viewsets: Child viewsets to include. Declared as a ``viewprop`` so
            subclasses can override it with a ``@viewprop`` method that returns
            a list lazily (avoiding import-time circular dependencies), while
            still allowing the value to be set on instances via ``__init__``
            kwargs or direct assignment.
    ...
    @viewprop
    def viewsets(self) -> list[BaseViewset] | None:  # type: ignore[override]
        """Child viewsets — override with @viewprop in subclasses for lazy imports."""
        return None
    ...
    def _get_urls(self) -> list[URLPattern | URLResolver]:
        ...
        ``RoutableComponent`` instances in ``viewsets`` are treated specially:
        they expose a single URL pattern via ``route_path``/``route_name`` and
        are included *flat* (no sub-namespace prefix) so that the generated URL
        is ``<parent_prefix><route_path>`` rather than
        ``<parent_prefix><app_name>/<route_path>``.

        Returns:
            list[Union[URLPattern, URLResolver]]: List of URL patterns
        """
        # Avoid a hard import cycle — import lazily.
        from django_osoul.site.routes.components import RoutableComponent
        ...
        for viewset in self.viewsets or []:
            # RoutableComponent: single-route component — include flat, no sub-namespace.
            if isinstance(viewset, RoutableComponent):
                viewset.__dict__["_parent"] = self
                view = type(viewset).as_view()
                name = viewset.route_name or camel_case_to_underscore(
                    viewset.__class__.__name__
                )
                urlpatterns.append(path(viewset.route_path, view, name=name))
                continue
