...
``django_osoul.comp.registry`` produces the same render output as the
Django builtin include machinery.

NOTE on import order: ``django_osoul.comp.configuration.conf`` touches
``settings.DEBUG`` at MODULE IMPORT TIME (it's a dataclass field
default), so we must configure Django BEFORE importing any
``django_osoul.comp`` submodule. The fixture below enforces that.
...
def _boot_django(test_templates_dir: Path) -> None:
    """Configure Django settings BEFORE importing django_osoul registries.
    ...
    **mutate** the existing ``TEMPLATES[0]`` to add our test directory
    ...
    if not settings.configured:
        ...
        if templates:
            ...
            if "django_osoul.comp.templatetags.components" not in _builtins:
                _builtins.append("django_osoul.comp.templatetags.components")
        else:
...
def reset_components():
    from django_osoul.comp.core._init import components  # imports after settings
...
def test_register_include_path_under_root_returns_list():
    from django_osoul.comp.registry import register_include_paths
