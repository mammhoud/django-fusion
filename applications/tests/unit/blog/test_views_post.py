...
class TestBlogPostListViewTagURL:
    ...
    def test_clean_url_filters_to_tagged_posts(
        self, client, db, post_python, post_django, tag_python
    ):
        url = reverse("blog:tag_filter", kwargs={"tag_slug": "python"})
        response = client.get(url)
...
class TestBlogPostListViewTagQueryParam:
    ...
    def test_no_tag_param_returns_all_published(
        self, client, db, post_python, post_django, post_draft
    ):
...
class TestBlogPostListViewCategoryURL:
    ...
    def test_clean_url_filters_to_categorized_posts(
        self, client, db, post_research, post_news, category_research
    ):
        url = reverse("blog:category_filter", kwargs={"category_slug": "research"})
        response = client.get(url)
        posts = list(response.context["posts"])
    ...
    def test_clean_url_sets_current_category_in_context(
        self, client, db, category_research, post_research
    ):
        url = reverse("blog:category_filter", kwargs={"category_slug": "research"})
        response = client.get(url)
        assert response.context["current_category"] == "research"

    def test_clean_url_sets_current_category_obj_in_context(
        self, client, db, category_research, post_research
    ):
        url = reverse("blog:category_filter", kwargs={"category_slug": "research"})
        response = client.get(url)
