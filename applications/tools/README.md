...
Nginx — see `applications/proxy/configs/nginx/default.conf.template`).
...
├── docus/           Docs site (was in proxy/nginx compose)
...
| `tools.structa.cloud/docs/` | Docus | `docus` | 3000 |
...
`applications/proxy/configs/traefik/dynamic/tools.yml`.
...
- **affine/** + **docus/** — migrated out of `applications/proxy/` so the proxy
  directory owns only Traefik/Nginx/certs.
