...
| ------------- | --------------------- | ----------------- | ------------ |
| `lms-demo`    | `lms-web`             | `lms-worker`      | `lms-demo`   |
...
(`ctc-research-worker`, `lms-demo-worker`, `vresume-worker`) used for the
...
```
celery -A tasks.celery:app worker --loglevel=info -Q lms-demo,ctc-research,vresume --hostname=shared-worker@%h
```
...
(`core/configs/base/celery.py`) already pins each `@shared_task` to the
...
| --- | --- | --- | --- | --- |
| `structa.cloud`, `*.structa.cloud` | LMS web handlers (`POST /learning/...`, `POST /api/...`) | `lms-demo` | `lms-demo` | `enrollment.send_welcome`, `certificate.render_pdf`, `invoice.refresh` |
...
`CELERY_TASK_DEFAULT_QUEUE`. The shared-worker `-Q lms-demo,ctc-research,vresume`
...
| ------------ | --------------------- | ----- |
| `lms-demo`   | `DB_NAME_LMS`, `LMS_DEMO_HOST`, `STRIPE_KEY_LMS` (if used) | Already namespaced at compose level |
| `ctc-research` | `DB_NAME_CTC`, `CTC_RESEARCH_HOST`              | Same           |
| `vresume`    | `DB_NAME_VRESUME`, `VRESUME_HOST`               | Same           |

If a site-specific value sits in `core/<site>/settings/` rather than env, the
...
### Phase 4 — Cleanup
1. Comment out per-site worker services in `core/lms-demo/docker-compose.yml`, `core/ctc-research/docker-compose.yml`, `core/VResume/docker-compose.yml`.
...
4. Update `core/Makefile` `docker-redeploy-with-worker` to **also** restart `shared-worker`.
...
```bash
for s in vresume ctc-research lms-demo; do
  docker compose -f core/$s/docker-compose.yml up -d --no-deps $(make -C core -s DOCKER_WORKER=${s}-worker show-config 2>/dev/null || echo ${s}-worker)
done
...
| 2 | All 3 queues visible | `docker logs shared-worker 2>&1 | rg "ready" | rg -c "lms-demo|ctc-research|vresume"` |
| 2 | Tasks delivered | `docker exec shared-worker celery -A tasks.celery:app inspect ping -d celery@lms-demo` |
| 3 | Queue draining | `redis-cli -n 1 LLEN lms-demo ctc-research vresume` |
