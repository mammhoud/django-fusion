...
# Lazy imports to avoid Django setup issues at module level
def _get_role_hierarchy_manager():
    from apps.accounts.services.groups import RoleHierarchyManager
    return RoleHierarchyManager

def _get_group_access_control():
...
class TestGroupAccessControl(TestCase):
    ...
    def setUp(self):
        ...
        from apps.accounts.services.groups import RoleHierarchyManager
        self.manager = RoleHierarchyManager()
        self.GroupAccessControl = GroupAccessControl
    ...
    def test_check_group_access(self):
        ...
        self.assertTrue(
            self.GroupAccessControl.check_group_access(self.user, ["supervisor"])
        )
        self.assertTrue(self.GroupAccessControl.check_group_access(self.user, ["user"]))
        self.assertFalse(
            self.GroupAccessControl.check_group_access(self.user, ["admin"])
        )
    ...
    def test_check_role_access(self):
        ...
        self.assertTrue(self.GroupAccessControl.check_role_access(self.user, "supervisor"))
        self.assertTrue(self.GroupAccessControl.check_role_access(self.user, "user"))
        self.assertFalse(self.GroupAccessControl.check_role_access(self.user, "admin"))

    def test_get_accessible_groups(self):
        """Test getting accessible groups."""
        self.manager.assign_user_to_role(self.user, "supervisor")

        groups = self.GroupAccessControl.get_accessible_groups(self.user)
    ...
    def test_superuser_access(self):
        ...
        self.assertTrue(
            self.GroupAccessControl.check_group_access(superuser, ["admin"])
        )
        self.assertTrue(
            self.GroupAccessControl.check_group_access(superuser, ["supervisor"])
        )
        self.assertTrue(self.GroupAccessControl.check_group_access(superuser, ["user"]))
...
class TestCreateGroupsFromCSV(TestCase):
    ...
    def setUp(self):
        from apps.accounts.services.groups import RoleHierarchyManager
        self.manager = RoleHierarchyManager()
        # Record pre-existing groups (e.g. Wagtail's Moderators/Editors)
        self.initial_group_count = Group.objects.count()
    ...
    def test_create_groups_from_csv(self):
        ...
        try:
            ...
            # Verify groups were created (accounting for pre-existing groups)
            self.assertEqual(Group.objects.count(), self.initial_group_count + 3)
        finally:
    ...
    def test_hierarchical_roles_in_csv(self):
        ...
        try:
            ...
            # Verify groups were created (accounting for pre-existing groups)
            self.assertEqual(Group.objects.count(), self.initial_group_count + 2)
