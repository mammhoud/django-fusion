...
if [ -f /root/site/websites/application/proxy/configs/acme.json ]; then
    test_pass "Traefik ACME store (acme.json) exists"
else
    test_warn "ACME store not found at /root/site/websites/application/proxy/configs/acme.json"
fi

if [ -f /root/site/websites/application/proxy/configs/acme.json ]; then
    ACME_PERMS=$(stat -c %a /root/site/websites/application/proxy/configs/acme.json 2>/dev/null || echo "?")
    test_pass "ACME store permissions: ${ACME_PERMS} (expect 600)"
fi

if [ -d /root/site/websites/application/proxy/data/certs ]; then
    CERT_COUNT=$(find /root/site/websites/application/proxy/data/certs -maxdepth 1 -type f 2>/dev/null | wc -l)
...
CONFIG_FILES=(
    "/root/site/websites/application/proxy/configs/traefik/dynamic.yml"
    "/root/site/websites/application/proxy/docker-compose.traefik.yml"
    "/root/site/websites/application/proxy/docker-compose.warehouse.yml"
    "/root/site/websites/application/proxy/docker-compose.nginx.yml"
