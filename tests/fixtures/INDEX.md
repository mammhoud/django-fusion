...
python manage.py --site=ctc loaddata projects/Clients/ctc-research/assets/fixtures/auth/user_dummy.json

# Load from tests/fixtures
python manage.py --site=ctc loaddata tests/fixtures/lms/courses.json
```
...
node assets/scripts/workspace.mjs load-dumps --site ctc
```
...
```bash
for fixture in projects/Clients/ctc-research/assets/fixtures/**/*.json; do
  python manage.py --site=ctc loaddata "$fixture"
done
```

---

> **Two copies.** This file is byte-identical in the root checkout and in the
> `structa.cloud` submodule. A fix must land in both or one goes stale.

## Fixtures by Site

### ctc-research (medical research site)

**Location:** `projects/Clients/ctc-research/assets/fixtures/`
...
```bash
python manage.py --site=ctc loaddata projects/Clients/ctc-research/assets/fixtures/auth/user_dummy.json
python manage.py --site=ctc loaddata projects/Clients/ctc-research/assets/fixtures/sites/site_dummy.json
python manage.py --site=ctc loaddata projects/Clients/ctc-research/assets/fixtures/test/locales.json
python manage.py --site=ctc loaddata projects/Clients/ctc-research/assets/fixtures/test/core-data.json
python manage.py --site=ctc loaddata tests/fixtures/lms/courses.json
```

---

### structa.cloud (unified product)

**Location:** `projects/Clients/structa.cloud/tests/fixtures/`
...
|-----------|-------|---------|
| `auth/` | `group_dummy.json`, `user_dummy.json` | Test users and groups |
| `sites/` | `site_dummy.json` | Django sites framework config (dummy host: `localhost`) |

See `tests/fixtures/lms-demo/README.md` for details.

---

### Retired: VResume (portfolio)

VResume was merged into the unified product. Its fixture directory and pointer
README were removed 2026-09-21 (deletion-manifest DOC-0036), and no `vresume`
site identity is loaded any more. Portfolio fixtures, if reintroduced, belong to
the unified product's set above.

---

## Shared LMS Fixtures

`tests/fixtures/lms/` contains copies of the ctc-research LMS fixtures for use in
...
python manage.py --site=ctc dumpdata lms.Course --indent 2 > tests/fixtures/lms/courses.json

# Export auth data
python manage.py --site=ctc dumpdata auth.User auth.Group --indent 2 \
    > projects/Clients/ctc-research/assets/fixtures/auth/user_dummy.json

# Export full database (large — use for production snapshots)
python manage.py --site=ctc dumpdata --indent 2 --natural-foreign --natural-primary \
    > projects/Clients/ctc-research/assets/fixtures/dump-data.json
```
