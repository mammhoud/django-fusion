...
def test_url(url: str, timeout: int = 10) -> Tuple[bool, int, str]:
    """
    Test a URL and return success status, status code, and message.
...
def test_health_endpoint(base_url: str, endpoint: str) -> Tuple[bool, str]:
    """
...
def main():
    ...
    for group_name, urls in DOMAINS_TO_TEST.items():
        ...
        for url in urls:
            success, status_code, message = test_url(url)
    ...
    for base_url, endpoint in HEALTH_ENDPOINTS.items():
        success, message = test_health_endpoint(base_url, endpoint)
    ...
    for base_url in ['http://localhost:5080', 'http://localhost:5081']:
    ...
    passed_tests += sum(1 for base_url, endpoint in HEALTH_ENDPOINTS.items() if test_health_endpoint(base_url, endpoint)[0])
