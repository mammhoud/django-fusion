...
def test_component_manifest_cache_tracks_path_and_file_changes(tmp_path):
    ...
    with override_settings(
        ...
        FUSION_ASSET_PIPELINE={"components": {"manifest_path": str(first)}},
    ):
    ...
    with override_settings(
        STATIC_ROOT=str(tmp_path),
        FUSION_ASSET_PIPELINE={"components": {"manifest_path": str(second)}},
    ):
...
def test_manifest_command_uses_configured_path_then_standalone_fallback(tmp_path, monkeypatch):
    ...
    with override_settings(
        STATIC_ROOT="",
        FUSION_ASSET_PIPELINE={"components": {"manifest_path": str(configured)}},
    ):
        command_module.Command().handle(output=None)

    with override_settings(STATIC_ROOT="", FUSION_ASSET_PIPELINE={}):
        command_module.Command().handle(output=None)
