...
├── docker-compose.applications.yml  ← includes all per-site compose files
├── docker-compose.tasks.yml         ← Celery worker/beat definitions
└── docker-compose.docs.yml           ← documentation server

core/compose/
├── Dockerfile                        ← shared Django site image
├── entrypoint                        ← container entrypoint script
├── start                             ← runtime start script
├── rqworker-start                    ← RQ worker start script
└── flower                            ← Celery Flower script

core/ctc-research/docker-compose.yml  ← per-site service definition
core/lms-demo/docker-compose.yml     ← per-site service definition
core/VResume/docker-compose.yml     ← per-site service definition
core/tinker/docker-compose.yml      ← template customizer service

applications/databases/docker-compose.yml  ← shared DB (postgres) + cache (redis)
applications/proxy/docker-compose.yml      ← Traefik / Nginx proxy stack
...
`applications/compose/docker-compose.warehouse.yml` defines the central database and cache containers
...
`applications/proxy/docker-compose.traefik.yml` defines:

- **structa-proxy** — Traefik v3 reverse proxy with ACME TLS, HTTP→HTTPS redirect, and the
  Traefik dashboard. Reads dynamic configuration from `applications/proxy/traefik/dynamic/`.

### Layer 2: Per-Site Services

Each site's `docker-compose.yml` defines its own services and networks. Shared networks and volumes are declared in the root `docker-compose.yml`.
...
```bash
# Start only ctc-research (still requires databases to be running)
docker compose -f core/ctc-research/docker-compose.yml up
...
### Production (applications/compose/docker-compose.warehouse.yml)
...
## Warehouses vs applications/compose/warehouse

These serve different purposes:

| | `warehouses/` | `applications/compose/docker-compose.warehouse.yml` |
|---|---|---|
| **Purpose** | Local development | Production / CI |
| **Image** | `postgres:14-alpine` (official) | Custom `applications/databases/postgres/Dockerfile` |
...
- `applications/compose/docker-compose.warehouse.yml` → production and CI. Uses the custom `structa-db`
...
docker compose -f applications/compose/docker-compose.warehouse.yml up -d
