...
**Location**: `/root/site/websites/applications/proxy/traefik/`
...
**Path**: `/root/site/websites/applications/proxy/traefik/certs/`

### Certificate Source Files

**Traefik ACME JSON**:
```
/root/site/websites/applications/proxy/traefik/acme/acme.json
- Size: ~13KB
...
```
/root/site/websites/applications/proxy/traefik/certs/
...
**Location**: `/root/site/websites/applications/proxy/scripts/cert-backup.sh`
...
1. **List available backups**:
   ```bash
   cd /root/site/websites/applications/proxy/scripts
   bash cert-backup.sh list
   ```
...
1. **Regenerate new certificates**:
   ...
   bash applications/proxy/traefik/generate-certs.sh
   ```
...
0 2 * * * cd /root/site/websites/applications/proxy/scripts && bash cert-backup.sh backup >> /var/log/traefik-backup.log 2>&1

# Add this line (cleanup old backups every Sunday at 3 AM, keep last 30)
0 3 * * 0 cd /root/site/websites/applications/proxy/scripts && bash cert-backup.sh cleanup 30 >> /var/log/traefik-backup.log 2>&1
```
...
BACKUP_DIR="/root/site/websites/applications/proxy/traefik/certs"
...
ls -la /root/site/websites/applications/proxy/traefik/certs/
...
ls -la applications/proxy/traefik/certs/

# Check ACME JSON
ls -la applications/proxy/traefik/acme/acme.json

# Verify certificate validity
openssl x509 -in applications/proxy/traefik/certs/ctc-research.crt -text -noout | grep -E "Issuer|Subject|Expires"
```
...
1. Check certificate expiry dates:
   ```bash
   for cert in applications/proxy/traefik/certs/*.crt; do
     echo "=== $(basename $cert) ==="
...
2. List all backups:
   ```bash
   bash applications/proxy/scripts/cert-backup.sh list
   ```

3. Verify backup integrity:
   ```bash
   for backup in applications/proxy/traefik/certs/*.tar.gz; do
...
1. Extract test backup:
   ...
   tar -xzf applications/proxy/traefik/certs/production-certs-*.tar.gz -C /tmp/cert-test
   ```
...
**Location**: `/root/site/websites/applications/proxy/traefik/certs/`
**Retention**: 30 days
...
```bash
#!/bin/bash
aws s3 sync /root/site/websites/applications/proxy/traefik/certs/ \
