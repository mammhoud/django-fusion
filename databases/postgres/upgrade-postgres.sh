...
start_stack() {
    local default_image_tag="${1:-${LATEST_IMAGE:-latest}}"
    local files
    files=$(compose_files)

    # shellcheck disable=SC2086
    LATEST_IMAGE="$default_image_tag" docker compose --env-file "$ENV_FILE" $files up -d --remove-orphans --wait --wait-timeout 120
}
...
rollback_postgres() {
    ...
    CURRENT_DEFAULT_IMAGE_TAG=$(current_default_image_tag)
    ...
    log "Current Coolify image tag: ${CURRENT_DEFAULT_IMAGE_TAG:-latest}"
    ...
    start_stack "$CURRENT_DEFAULT_IMAGE_TAG" >>"$LOGFILE" 2>&1 || fail "Could not start Coolify stack after rollback. See ${LOGFILE}."
...
upgrade_postgres() {
    ...
    if docker volume inspect "$TARGET_VOLUME" >/dev/null 2>&1; then
        fail "Target volume '${TARGET_VOLUME}' already exists. Set DEFAULT_POSTGRES_TARGET_VOLUME to a new name or remove the old failed target volume."
    fi
    ...
    start_stack "$CURRENT_DEFAULT_IMAGE_TAG" >>"$LOGFILE" 2>&1 || fail "Could not start Coolify stack with upgraded PostgreSQL. See ${LOGFILE}."

    log "Coolify internal PostgreSQL upgrade completed successfully."
    print_rollback_instructions
}

if [ "${DEFAULT_POSTGRES_UPGRADE_SOURCE_ONLY:-false}" = "true" ] && [ "${BASH_SOURCE[0]}" != "$0" ]; then
    return 0
fi
